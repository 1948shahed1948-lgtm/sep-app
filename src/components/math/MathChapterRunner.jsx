...
function InfoCard({ q, onNext, theme }) {
  return (
    <div className="space-y-4">
      <div className={`rounded-xl ${theme.lightBg} border-2 ${theme.lightBorder} p-4 flex items-start gap-3`}>
        <div className={`flex-shrink-0 w-8 h-8 rounded-full ${theme.accent} flex items-center justify-center`}>
          <Lightbulb className="w-4 h-4 text-white" />
        </div>
        <div>
          <p className={`text-xs font-bold ${theme.accentText} mb-1`}>معلومة من الكتاب 📖</p>
          <p dir="rtl" className={`text-sm ${theme.darkText} leading-relaxed font-medium`}>{q.question_ar}</p>
        </div>
      </div>
      <Button onClick={onNext} className={`w-full ${theme.btn} gap-2`}>فهمت ← السؤال التالي <ArrowRight className="w-4 h-4" /></Button>
    </div>
  );
}

function QuestionRenderer({ q, onAnswer, isAnswered, theme }) {
      ...
      {q.options_ar.map((opt, i) => {
        ...
        return (
          <button key={i} disabled={show} onClick={() => handleMC(i)}
            ...
            <span className="flex items-center gap-2">{show && isCorrect && <CheckCircle2 className="w-4 h-4 text-emerald-600 flex-shrink-0" />}{show && isPicked && !isCorrect && <XCircle className="w-4 h-4 text-red-500 flex-shrink-0" />}{opt}</span>
          </button>
        );
      })}
    </div>
  ...
  if (q.type === 'matching') return (
    <div className="space-y-3"><p className="text-xs text-slate-500 text-center">اضغط من العمود الأيمن ثم من الأيسر:</p><div className="grid grid-cols-2 gap-3"><div className="space-y-2">{q.pairs_ar.map((pair, i) => (<button key={i} onClick={() => handleMatchPick('right', i)} className={`w-full text-right px-3 py-2.5 rounded-lg border-2 text-xs font-medium transition-all ${matchSel?.side === 'right' && matchSel?.idx === i ? theme.matchActive : 'border-slate-200 bg-white ' + theme.matchHover} ${connections[i] !== undefined ? 'border-emerald-400 bg-emerald-50' : ''}`}>{pair.right}</button>))}</div><div className="space-y-2">{q.pairs_ar.map((pair, i) => (<button key={i} onClick={() => handleMatchPick('left', i)} className={`w-full text-right px-3 py-2.5 rounded-lg border-2 text-xs font-medium transition-all ${matchSel?.side === 'left' && matchSel?.idx === i ? theme.matchActive : 'border-slate-200 bg-white ' + theme.matchHover} ${Object.values(connections).includes(i) ? 'border-emerald-400 bg-emerald-50' : ''}`}>{pair.left}</button>))}</div></div></div>
  );

  if (q.type === 'ordering' && orderItems) {
    const checked = selected === 'checked';
    return (
      <div className="space-y-3"><p className="text-xs text-slate-500">استخدم الأسهم لإعادة الترتيب:</p>{orderItems.map((item, i) => (<div key={item} className="flex items-center gap-2 bg-white border-2 border-slate-200 rounded-xl px-3 py-2.5"><span className={`w-6 h-6 rounded-full ${theme.badgeBg} flex items-center justify-center text-xs font-bold ${theme.badgeText} flex-shrink-0`}>{i + 1}</span><span className="flex-1 text-xs sm:text-sm font-medium text-slate-800 text-right">{item}</span><div className="flex flex-col gap-0.5"><button disabled={i === 0 || checked} onClick={() => moveItem(i, i - 1)} className="text-slate-400 hover:text-slate-700 disabled:opacity-30 text-xs">▲</button><button disabled={i === orderItems.length - 1 || checked} onClick={() => moveItem(i, i + 1)} className="text-slate-400 hover:text-slate-700 disabled:opacity-30 text-xs">▼</button></div></div>))}{!checked && <Button onClick={checkOrder} className={`w-full ${theme.btn}`}>تحقق من الترتيب ✓</Button>}</div>
    );
  }
  return null;
}

function LessonRunner({ lesson, onBack, theme }) {
    ...
    return (
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center py-10 px-4 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          ...
          <Button onClick={onBack} className={`${theme.btn} gap-2`}><ArrowRight className="w-4 h-4" />العودة</Button>
        </div>
      </motion.div>
    );
  }
  ...
  return (
    <div ref={containerRef} className="space-y-4">
      <div className="flex items-center gap-3">
        <div className="flex-1 bg-slate-100 rounded-full h-2"><div className={`${theme.progress} h-2 rounded-full transition-all duration-500`} style={{ width: `${(idx / questions.length) * 100}%` }} /></div>
        <span className="text-xs font-medium text-slate-500">{idx + 1} / {questions.length}</span>
        <span className={`text-xs font-bold ${theme.accentText}`}>✅ {score}</span>
      </div>
      <AnimatePresence mode="wait">
        <motion.div key={q.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 space-y-4">
          {isInfo ? <InfoCard q={q} onNext={next} theme={theme} /> : (
            <>
              <div className="flex items-start gap-2"><span className={`text-xs ${theme.badgeBg} ${theme.badgeText} rounded-full px-2 py-0.5 font-bold flex-shrink-0 mt-0.5`}>{idx + 1}</span><p className="text-base font-bold text-slate-800 leading-relaxed">{q.question_ar}</p></div>
              <QuestionRenderer q={q} onAnswer={handleAnswer} isAnswered={answered} theme={theme} />
              {answered && (
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
                  ...
                  <Button onClick={next} className={`w-full mt-3 ${theme.btn} gap-2`}>{idx + 1 >= questions.length ? <><Trophy className="w-4 h-4" />إنهاء الدرس</> : <>السؤال التالي<ArrowRight className="w-4 h-4" /></>}</Button>
                </motion.div>
              )}
            </>
          )}
...
export default function MathChapterRunner({ units, total, title, subtitle, emoji, theme, backTo = '/question-bank/grades' }) {
  ...
  if (activeUnit !== null && activeLesson !== null) {
    ...
    return (
      <div className={`min-h-screen bg-gradient-to-br ${theme.pageBg} p-4 sm:p-6`} dir="rtl">
        <div className="max-w-2xl mx-auto space-y-4">
          ...
          <LessonRunner lesson={lesson} onBack={() => setActiveLesson(null)} theme={theme} />
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-2xl mx-auto space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {unit.lessons.map((lesson, i) => (
              <motion.button key={lesson.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} onClick={() => setActiveLesson(i)} className="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 text-right hover:shadow-md hover:border-slate-300 transition-all flex items-center gap-3">
                ...
                <BookOpen className={`w-4 h-4 ${theme.accentText} flex-shrink-0`} />
              </motion.button>
            ))}
          </div>
        </div>
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br ${theme.pageBg}`} dir="rtl">
      <div className="relative overflow-hidden" style={{ background: theme.heroGradient }}>
        ...
        <div className="relative max-w-3xl mx-auto px-4 py-10 sm:py-14 text-center text-white">
          <Link to={backTo} className="inline-flex items-center gap-1.5 text-xs text-white/60 hover:text-white/90 mb-4 transition-colors"><Home className="w-3.5 h-3.5" />بنك الأسئلة</Link>
          ...
          <div className="flex justify-center gap-4 mt-5 text-xs flex-wrap">
            <div className="bg-white/10 rounded-xl px-4 py-2"><p className="font-bold text-lg">{units.length}</p><p className="text-white/70">وحدات</p></div>
            <div className="bg-white/10 rounded-xl px-4 py-2"><p className="font-bold text-lg">{units.reduce((s, u) => s + u.lessons.length, 0)}</p><p className="text-white/70">دروس</p></div>
            <div className="bg-white/10 rounded-xl px-4 py-2"><p className="font-bold text-lg">{total}</p><p className="text-white/70">سؤالاً</p></div>
          </div>
        </div>
      </div>
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-4">
        {units.map((unit, i) => (
          <motion.button key={unit.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} onClick={() => setActiveUnit(i)} className="w-full bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-md hover:border-slate-300 transition-all text-right">
            <div className="flex items-center gap-4 p-5">
              ...
              <div className="flex-1"><h2 className="text-base font-bold text-slate-800">{unit.title_ar}</h2><p className="text-xs text-slate-500 mt-0.5">{unit.subtitle_ar}</p><div className="flex items-center gap-3 mt-2"><span className={`text-xs ${theme.badgeBg} ${theme.badgeText} rounded-full px-2 py-0.5`}>{unit.lessons.length} درس</span><span className="text-xs bg-slate-100 text-slate-600 rounded-full px-2 py-0.5">{unit.lessons.reduce((s, l) => s + l.count, 0)} سؤالاً</span></div></div>
              <Star className={`w-5 h-5 ${theme.starColor} flex-shrink-0`} />
            </div>
          </motion.button>
        ))}
      </div>

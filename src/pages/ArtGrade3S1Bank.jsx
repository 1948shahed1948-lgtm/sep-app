...
import {
  ...
  Trophy, Star, RotateCcw, ArrowLeft,
} from 'lucide-react';
import {
  ART_G3_S1_META,
  ART_G3_S1_UNITS,
  ART_G3_S1_QUESTIONS,
  TOTAL_QUESTIONS_G3_ART,
} from '@/lib/artGrade3S1Bank';
        ...
        return (
          <button key={i} disabled={!!answered} onClick={() => !answered && onAnswer(i, isCorrect)}
            className={`w-full text-right px-4 py-3 rounded-xl border-2 transition-all flex items-center gap-3 disabled:cursor-default
              ${showOk ? 'border-emerald-400 bg-emerald-50' : showBad ? 'border-red-400 bg-red-50' : 'border-slate-200 bg-white hover:border-amber-400 hover:bg-amber-50/30'}`}>
            <div className="w-7 h-7 rounded-full flex-shrink-0 flex items-center justify-center text-xs font-bold text-white"
              style={{ background: ['#6366f1','#b45309','#0891b2','#059669'][i] }}>
              {['أ','ب','ج','د'][i]}
            </div>
        ...
        return (
          <button key={val} disabled={!!answered} onClick={() => !answered && onAnswer(val, val === q.correctAnswer)}
            className={`py-5 rounded-2xl border-2 font-bold text-base transition-all
              ${showOk ? 'border-emerald-400 bg-emerald-50 text-emerald-700' : showBad ? 'border-red-400 bg-red-50 text-red-700' : 'border-slate-200 bg-white hover:border-amber-400'}`}>
    ...
    <div className="space-y-2">
      ...
      {!submitted && (
        <button onClick={() => { setSubmitted(true); onAnswer(correct); }}
          className="w-full mt-2 rounded-xl bg-primary text-white font-bold py-2.5 text-sm">
          تحقق من الترتيب ✓
        </button>
      )}
      {submitted && (
...
function ClassifyQuestion({ q, onAnswer }) {
  ...
  return (
    <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {pool.map(it => (
              <span key={it.id} onClick={() => setSel(it)}
                className={`px-3 py-1.5 rounded-lg border cursor-pointer text-sm ${sel?.id === it.id ? 'border-amber-500 bg-amber-50 font-bold' : 'bg-white border-slate-300 hover:border-amber-400'}`}>
                {it.text}
              </span>
            ))}
          </div>
      ...
      <div className={`grid gap-3 grid-cols-${Math.min(q.categories.length, 2)}`}>
        {q.categories.map((cat, ci) => (
          <div key={ci} onClick={() => moveTo(ci)} className="rounded-xl border-2 border-slate-200 p-3 min-h-24 cursor-pointer hover:border-amber-300 transition-colors">
...
function GoldenBox({ text }) {
  return (
    <div className="rounded-xl p-3 border-2 mt-3" style={{ borderColor: '#b45309', background: 'linear-gradient(135deg,#fef9e7,#fff8dc)' }}>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={idx} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}
          ...
          <div className="px-4 pt-4 pb-2 border-b border-slate-100 flex flex-wrap gap-1.5">
            ...
            <span className="text-xs px-2 py-0.5 rounded-full bg-indigo-100 text-indigo-700">{TYPE_LABEL[q.type]}</span>
          </div>
          <div className="px-4 py-3">
            <p className="font-body font-bold text-slate-900 mb-4 text-base leading-snug">{q.question}</p>
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
      ...
      <div className="rounded-2xl p-8 text-center text-white"
        ...
        <Crown className="w-10 h-10 mx-auto mb-3 text-amber-200" />
...
// ── Main Page ─────────────────────────────────────────────────────────────
export default function ArtGrade3S1Bank() {
  ...
  const lessonQuestions = useMemo(() => {
    ...
    return ART_G3_S1_QUESTIONS.filter(q => q.lesson === selectedLesson.id);
  }, [selectedLesson]);
  ...
  const handleExit = (finalScore) => {
    setScore(finalScore);
    setView('result');
  };

  const totalCorrect = useMemo(() => {
    const allQ = ART_G3_S1_QUESTIONS;
    return { total: allQ.length };
  }, []);
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-5 space-y-5">
        ...
        <div className="rounded-2xl p-5 text-white" style={{ background: `linear-gradient(135deg,${selectedUnit.color},${selectedUnit.color}99)` }}>
          ...
          <p className="text-white/60 text-xs mt-1">{selectedUnit.lessons.length} دروس · {ART_G3_S1_QUESTIONS.filter(q => q.unit === selectedUnit.id).length} سؤال</p>
        </div>
        <div className="space-y-3">
          {selectedUnit.lessons.map(lesson => {
            const lQ = ART_G3_S1_QUESTIONS.filter(q => q.lesson === lesson.id);
            return (
              <button key={lesson.id} onClick={() => openLesson(selectedUnit, lesson)}
                className="w-full rounded-2xl p-4 bg-white border-2 border-slate-200 hover:border-amber-400 transition-all text-right flex items-center gap-3 hover:shadow-md">
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-5 space-y-6">
      ...
      <div className="relative rounded-3xl overflow-hidden p-6 text-center shadow-xl"
        style={{ background: 'linear-gradient(135deg,#1a1035,#3d1c6e,#b45309)' }}>
        ...
        <h1 className="text-white font-heading font-bold text-xl mb-1">التربية الفنية</h1>
        <p className="text-white/70 text-sm">{ART_G3_S1_META.grade} · {ART_G3_S1_META.semester}</p>
        <p className="text-white/40 text-xs mt-1">{ART_G3_S1_META.year}</p>
        <div className="mt-3 flex justify-center gap-3 flex-wrap">
          <div className="bg-white/10 px-3 py-1.5 rounded-full text-xs text-white/80">
            {toAr(ART_G3_S1_UNITS.length)} وحدات
          </div>
          <div className="bg-amber-500/20 px-3 py-1.5 rounded-full text-xs text-amber-300">
            {toAr(TOTAL_QUESTIONS_G3_ART)} سؤال
          </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {ART_G3_S1_UNITS.map((unit, ui) => {
          const unitQ = ART_G3_S1_QUESTIONS.filter(q => q.unit === unit.id);
          return (
            <motion.button key={unit.id} onClick={() => { setSelectedUnit(unit); setView('unit'); }}
              ...
              <div className="relative">
                ...
                <div className="mt-2 flex flex-wrap gap-1">
                  {unit.lessons.map(l => (
                    <span key={l.id} className="text-[10px] bg-white/20 text-white px-2 py-0.5 rounded-full">
                      {l.title_ar.slice(0, 12)}...
                    </span>
                  ))}
                </div>
              </div>
      ...
      <div className="rounded-2xl bg-amber-50 border border-amber-200 p-4 text-center">
        ...
        <p className="text-amber-800 text-sm font-medium">{ART_G3_S1_META.publisher}</p>
        <p className="text-amber-600 text-xs mt-1">جميع الأسئلة مستخرجة من الكتاب المدرسي المعتمد</p>
      </div>
    </div>
  );
}
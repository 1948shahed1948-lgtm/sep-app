...
import { ArrowRight, CheckCircle2, Crown, BookOpen, ChevronRight, RotateCcw } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { useLang } from '@/lib/i18n';
import PageHeader from '@/components/layout/PageHeader';
import WrongAnswerTeacher from '@/components/aptitude/WrongAnswerTeacher';
import { useScrollTopOnChange } from '@/hooks/useScrollTopOnChange';
import { ART_G5_S2_UNITS, ART_G5_S2_TOTAL } from '@/lib/artGrade5S2Bank';
...
function OrderingQ({ q, onAnswer, answered }) {
  ...
  return (
    <div className="space-y-3" dir="rtl">
      ...
      <p className="text-xs text-slate-400">💡 استخدم الأسهم لترتيب العناصر</p>
      <div className="space-y-2">
        {items.map((item, i) => (
          <div key={item} className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2">
            <span className="w-6 h-6 rounded-full bg-amber-100 text-amber-700 text-xs font-bold flex items-center justify-center flex-shrink-0">{toAr(i+1)}</span>
            <span className="flex-1 text-sm text-slate-800">{item}</span>
            {!answered && !checked && (
              <div className="flex flex-col gap-0.5">
                <button onClick={() => moveUp(i)} className="text-slate-400 hover:text-amber-600 text-xs leading-none">▲</button>
                <button onClick={() => moveDown(i)} className="text-slate-400 hover:text-amber-600 text-xs leading-none">▼</button>
              </div>
            )}
          </div>
        ))}
...
function MatchingQ({ q, onAnswer, answered }) {
  ...
  const shuffledRight = useMemo(() => [...q.pairs.map(p => p.right)].sort(() => Math.random() - 0.5), []);
  const handleLeft = (l) => { if (checked) return; setSelected({ val: l }); };
  const handleRight = (r) => {
  ...
  return (
    <div className="space-y-3" dir="rtl">
      <p className="text-sm font-bold text-slate-700">{q.q}</p>
      <p className="text-xs text-slate-400">💡 اضغط يسارًا ثم اختَر ما يقابله يمينًا</p>
      <div className="grid grid-cols-2 gap-2">
        <div className="space-y-2">
          {leftItems.map((l) => (
            <button key={l} onClick={() => handleLeft(l)} disabled={checked}
              className={`w-full text-xs px-2 py-2 rounded-lg border-2 text-right transition-all ${selected?.val===l ? 'border-amber-500 bg-amber-50' : connections[l] ? 'border-emerald-400 bg-emerald-50' : 'border-slate-200 hover:border-amber-300'}`}>
              {l} {connections[l] && <span className="text-emerald-600 text-[10px]">←</span>}
            </button>
          ))}
        </div>
        <div className="space-y-2">
          {shuffledRight.map((r) => (
            <button key={r} onClick={() => handleRight(r)} disabled={checked}
              className={`w-full text-xs px-2 py-2 rounded-lg border-2 text-right transition-all ${selected ? 'border-amber-400 bg-amber-50 cursor-pointer' : 'border-slate-200'}`}>
              {r}
            </button>
          ))}
        </div>
...
function ClassifyQ({ q, onAnswer, answered }) {
  ...
  return (
    <div className="space-y-3" dir="rtl">
      <p className="text-sm font-bold text-slate-700">{q.q}</p>
      <p className="text-xs text-slate-400">💡 اضغط على العنصر لتختار فئته</p>
      {pool.length > 0 && (
        <div className="flex flex-wrap gap-2 p-3 bg-slate-50 rounded-xl border border-dashed border-slate-300">
          {pool.map((item) => (
            <div key={item.text} className="relative group">
              ...
              <div className="absolute top-7 right-0 z-10 hidden group-hover:flex flex-col gap-1 bg-white border border-slate-200 rounded-lg shadow-lg p-1 min-w-[160px]">
                {q.categories.map((cat) => (
                  <button key={cat} onClick={() => assign(item, cat)} className="text-xs px-2 py-1 rounded hover:bg-amber-50 text-right">{cat}</button>
                ))}
              </div>
            </div>
          ))}
      ...
      <div className={`grid gap-3 ${q.categories.length > 3 ? 'grid-cols-2' : 'grid-cols-2'}`}>
        {q.categories.map((cat, ci) => (
          <div key={cat} className="bg-slate-50 rounded-xl border-2 border-dashed border-slate-300 p-2 min-h-[60px]">
            ...
            <div className="flex flex-wrap gap-1">
              {bins[cat].map((item) => (
                <button key={item.text} onClick={() => removeFromBin(item, cat)} disabled={checked}
                  className={`text-xs px-2 py-1 rounded-lg border font-medium ${checked ? (item.correct_cat===ci ? 'bg-emerald-100 border-emerald-400 text-emerald-800' : 'bg-rose-100 border-rose-400 text-rose-800') : 'bg-amber-100 border-amber-300 text-amber-800'}`}>
                  {item.text}
                </button>
              ))}
            </div>
...
function WordOrderQ({ q, onAnswer, answered }) {
  ...
  const check = () => {
    const s = slots.join(' ');
    setChecked(true);
    onAnswer(s === q.correct_sentence ? '1' : '0');
  };
  const reset = () => { setPool([...q.words].sort(()=>Math.random()-0.5)); setSlots([]); setChecked(false); };
  return (
    <div className="space-y-3" dir="rtl">
      <p className="text-sm font-bold text-slate-700">{q.q}</p>
      <p className="text-xs text-slate-400">💡 اضغط الكلمات بالترتيب الصحيح</p>
      <div className="min-h-[48px] p-3 bg-slate-50 border-2 border-dashed border-slate-300 rounded-xl flex flex-wrap gap-2">
        {slots.map((w, i) => (
          <button key={i} onClick={() => removeWord(i)} disabled={checked}
            className={`px-3 py-1.5 rounded-lg text-sm font-bold border-2 ${checked ? (slots.join(' ')===q.correct_sentence ? 'border-emerald-400 bg-emerald-50 text-emerald-800' : 'border-rose-400 bg-rose-50 text-rose-800') : 'border-amber-400 bg-amber-50 text-amber-800'}`}>
            {w}
          </button>
        ))}
      ...
      <div className="flex flex-wrap gap-2">
        {pool.map((w, i) => (
          <button key={i} onClick={() => addWord(w)} className="px-3 py-1.5 rounded-lg text-sm font-medium border-2 border-slate-200 bg-white hover:border-amber-400 hover:bg-amber-50 transition-all">{w}</button>
        ))}
      </div>
      ...
      {checked && (
        <p className="text-xs text-slate-500 bg-slate-50 rounded-lg p-2">
          الجملة الصحيحة: <span className="font-bold text-emerald-700">{q.correct_sentence}</span>
        </p>
      )}
    </div>
  );
...
export default function ArtGrade5S2() {
  ...
  const unit = active ? ART_G5_S2_UNITS[active.unitIdx] : null;
    ...
    return (
      <div className="space-y-6" dir="rtl">
        <PageHeader
          ...
          breadcrumbs={[{ label: t('home'), path: '/' }, { label: 'التربية الفنية — الصف الخامس ف٢', path: '/art-grade5-s2' }, { label: lesson.title_ar }]}
        />
        <div className="max-w-2xl mx-auto space-y-5 px-4">
          ...
          <button onClick={backToUnits} className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-800 transition-colors">
            <ArrowRight className="w-4 h-4" /> عودة لكلِّ الوحدات
          </button>
          ...
          <AnimatePresence mode="wait">
            <motion.div key={qIdx} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}
              ...
              <div className="flex items-center gap-2 pb-2 border-b border-slate-100">
                ...
                <span className="text-xs text-slate-400">التربية الفنية — الصف الخامس الابتدائي — الفصل الثاني</span>
              </div>
          ...
          {answered && (
            <>
              ...
              <Button onClick={next} className="w-full gap-2 py-6 text-base font-bold">
                {qIdx < questions.length-1 ? 'السؤال التَّالي' : 'إنهاءُ الدَّرس'} <ArrowRight className="w-5 h-5" />
              </Button>
            </>
          )}
        </div>
  ...
  if (active && done) {
    ...
    return (
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div className="rounded-2xl p-8 text-center bg-gradient-to-br from-amber-600 to-orange-500 text-white">
          <Crown className="w-10 h-10 mx-auto mb-3 text-yellow-200" />
          <h2 className="text-3xl font-heading font-bold mb-1">{toAr(score)} / {toAr(questions.length)}</h2>
          <p className="text-white/80">{pct >= 80 ? 'إتقانٌ ممتازٌ 🏆' : pct >= 50 ? 'جيِّدٌ، واصِل 💪' : 'أعِد المحاولة 📖'}</p>
        ...
        <div className="flex gap-3">
          ...
          <Button onClick={backToUnits} className="flex-1 gap-2">وحداتٌ أخرى <ArrowRight className="w-4 h-4" /></Button>
        </div>
      </motion.div>
    );
  }
  ...
  return (
    <div className="space-y-8" dir="rtl">
      <PageHeader
        title="التربية الفنية — الصف الخامس الابتدائي · الفصل الثاني"
        subtitle={`${toAr(ART_G5_S2_UNITS.length)} وحداتٍ · ${toAr(ART_G5_S2_TOTAL)} سؤالًا — الحفر على الخشب · إعداد النول وتسديته · النسيج الشعبي`}
        breadcrumbs={[{ label: t('home'), path: '/' }, { label: 'التربية الفنية — الصف الخامس ف٢' }]}
      />
      <div className="max-w-4xl mx-auto px-4 space-y-6">
        <div className="grid grid-cols-3 gap-3">
          {[
            { n: toAr(ART_G5_S2_UNITS.length), l: 'وحدة' },
            { n: toAr(ART_G5_S2_UNITS.reduce((s,u)=>s+u.lessons.length,0)), l: 'درسًا' },
            { n: toAr(ART_G5_S2_TOTAL), l: 'سؤالًا' },
          ].map((s) => (

...
import { ART_G6_S1_UNITS, ART_G6_S1_TOTAL } from '@/lib/artGrade6S1Bank';
import WrongAnswerTeacher from '@/components/aptitude/WrongAnswerTeacher';

const COLORS = ['bg-blue-50 border-blue-300', 'bg-amber-50 border-amber-300', 'bg-emerald-50 border-emerald-300', 'bg-rose-50 border-rose-300'];
...
function QuestionRenderer({ q, onAnswer, isAnswered }) {
  ...
  if (q.type === 'classify' && dragItems !== null) {
    ...
    const catColors = ['border-blue-200 bg-blue-50', 'border-amber-200 bg-amber-50', 'border-purple-200 bg-purple-50'];
    const btnColors = ['bg-blue-500 hover:bg-blue-600', 'bg-amber-500 hover:bg-amber-600', 'bg-purple-500 hover:bg-purple-600'];
    return (
  ...
  if (q.type === 'matching') {
    return (
      <div className="space-y-3">
        <p className="text-xs text-slate-500 text-center">اضغط من العمود الأيمن ثم من الأيسر للربط:</p>
            ...
            {q.pairs_ar.map((p, i) => (
              <button key={i} onClick={() => handleMatchPick('right', i)}
                className={`w-full text-right px-3 py-2 rounded-lg border-2 text-xs font-medium transition-all
                  ...
                  ${connections[i] !== undefined ? 'border-blue-400 bg-blue-50' : ''}`}>{p.right}</button>
            ))}
          </div>
          <div className="space-y-2">
            {q.pairs_ar.map((p, i) => (
              <button key={i} onClick={() => handleMatchPick('left', i)}
                className={`w-full text-right px-3 py-2 rounded-lg border-2 text-xs font-medium transition-all
                  ...
                  ${Object.values(connections).includes(i) ? 'border-blue-400 bg-blue-50' : ''}`}>{p.left}</button>
            ))}
          </div>
        </div>
      </div>
  ...
  if (q.type === 'ordering' && orderItems) {
    ...
    return (
      <div className="space-y-3">
        ...
        {!checked && <Button onClick={checkOrder} className="w-full bg-blue-700 hover:bg-blue-800">تحقق من الترتيب ✓</Button>}
      </div>
    );
  }
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
              ...
              {lastCorrect && q.explanation_ar && (
                <div className="mt-3 rounded-xl bg-blue-50 border border-blue-100 p-3">
                  <p className="text-xs font-bold text-blue-700 mb-1">💡 شرح الإجابة:</p>
                  <p className="text-sm text-blue-900 leading-relaxed">{q.explanation_ar}</p>
                  {q.golden_rule_ar && (
...
export default function ArtGrade6S1() {
  const [activeUnit, setActiveUnit] = useState(null);
  const [activeLesson, setActiveLesson] = useState(null);

  if (activeUnit !== null && activeLesson !== null) {
    const unit = ART_G6_S1_UNITS[activeUnit];
    const lesson = unit.lessons[activeLesson];
    return (
      <div className="min-h-screen p-4 sm:p-6" dir="rtl" style={{ background: '#f8f6f0' }}>
            ...
            {unit.lessons.map((lesson, i) => (
              <motion.button key={lesson.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">{lesson.count} سؤالاً</p>
                </div>
  ...
  return (
    <div className="min-h-screen" dir="rtl" style={{ background: '#f8f6f0' }}>
      <div className="relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #1a3a6e 0%, #2d5ba8 50%, #c9a227 100%)' }}>
        ...
        <div className="relative max-w-3xl mx-auto px-4 py-10 sm:py-14 text-center text-white">
          ...
          <p className="text-white/80 text-sm">الفصل الدراسي الأول · الرسم + الزخرفة + الطباعة + الخزف</p>
          <div className="flex justify-center gap-4 mt-5 text-xs flex-wrap">
            <div className="bg-white/10 rounded-xl px-4 py-2 border border-white/20">
              <p className="font-bold text-lg">{ART_G6_S1_UNITS.length}</p>
              <p className="text-white/70">وحدات</p>
            </div>
            <div className="bg-white/10 rounded-xl px-4 py-2 border border-white/20">
              <p className="font-bold text-lg">{ART_G6_S1_UNITS.reduce((s, u) => s + u.lessons.length, 0)}</p>
              <p className="text-white/70">دروس</p>
            </div>
            <div className="bg-white/10 rounded-xl px-4 py-2 border border-white/20">
              <p className="font-bold text-lg">{ART_G6_S1_TOTAL}</p>

...
import { ART_G6_S2_UNITS as ART_UNITS_1, ART_G6_S2_TOTAL as ART_TOTAL_1 } from '@/lib/artGrade6S2Bank';
import { ART_G6_S2_UNITS_PART2, ART_G6_S2_TOTAL_PART2 } from '@/lib/artGrade6S2Bank_Part2';
import WrongAnswerTeacher from '@/components/aptitude/WrongAnswerTeacher';

const ART_G6_S2_UNITS = [...ART_UNITS_1, ...ART_G6_S2_UNITS_PART2];
const ART_G6_S2_TOTAL = ART_TOTAL_1 + ART_G6_S2_TOTAL_PART2;
...
// زخرفة إسلامية SVG خفيفة للخلفية
        ...
        <defs>
          <pattern id="islamicStar" width="60" height="60" patternUnits="userSpaceOnUse">
            <path d="M30 5 L37 22 L55 22 L41 33 L46 50 L30 40 L14 50 L19 33 L5 22 L23 22 Z"
              fill="none" stroke="#0f5132" strokeWidth="0.8" />
          </pattern>
        </defs>
    ...
    return (
      <div className="flex gap-3">
        {[{ label: '✅ نعم', val: true }, { label: '❌ لا', val: false }].map(({ label, val }) => {
          ...
          return (
            <button key={label} disabled={show} onClick={() => handleYesNo(val)}
      ...
      <div className="space-y-3">
        ...
        {orderItems.map((item, i) => (
          <div key={item} className="flex items-center gap-2 bg-white border-2 border-slate-200 rounded-xl px-3 py-2">
            <span className="w-6 h-6 rounded-full bg-emerald-100 flex items-center justify-center text-xs font-bold text-emerald-700 flex-shrink-0">{i + 1}</span>
        ...
        {!checked && <Button onClick={checkOrder} className="w-full bg-emerald-700 hover:bg-emerald-800">تحقق من الترتيب ✓</Button>}
      </div>
    );
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <div className="flex-1 bg-slate-100 rounded-full h-2">
          <div className="bg-emerald-600 h-2 rounded-full transition-all duration-500" style={{ width: `${(idx / questions.length) * 100}%` }} />
        </div>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={q.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
          className="bg-white rounded-2xl shadow-sm border border-emerald-100 p-5 space-y-4">
          ...
          {answered && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <Button onClick={next} className="w-full mt-3 bg-emerald-700 hover:bg-emerald-800 gap-2">
...
export default function ArtGrade6S2() {
  const [activeUnit, setActiveUnit] = useState(null);
  const [activeLesson, setActiveLesson] = useState(null);

  if (activeUnit !== null && activeLesson !== null) {
    const unit = ART_G6_S2_UNITS[activeUnit];
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {unit.lessons.map((lesson, i) => (
              <motion.button key={lesson.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                ...
                className="bg-white rounded-2xl shadow-sm border border-emerald-100 p-4 text-right hover:shadow-md hover:border-emerald-300 transition-all flex items-center gap-3">
                <div className="w-12 h-12 rounded-xl flex-shrink-0 bg-emerald-50 border border-emerald-200 flex items-center justify-center">
  ...
  return (
    <div className="min-h-screen" dir="rtl" style={{ background: '#faf8f0' }}>
      {/* الغلاف */}
      <div className="relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #0f5132 0%, #157347 50%, #c9a227 100%)' }}>
        <IslamicPattern />
        <div className="relative max-w-3xl mx-auto px-4 py-10 sm:py-14 text-center text-white">
          ...
          <h1 className="text-2xl sm:text-3xl font-black mb-2">التربية الفنية — الصف السادس</h1>
          <p className="text-white/80 text-sm">الفصل الدراسي الثاني · الزخارف المتشعبة + أشغال المعادن</p>
          <div className="flex justify-center gap-4 mt-5 text-xs flex-wrap">
            <div className="bg-white/10 rounded-xl px-4 py-2 border border-white/20">
              <p className="font-bold text-lg">{ART_G6_S2_UNITS.length}</p>
              <p className="text-white/70">وحدات</p>
            </div>
            <div className="bg-white/10 rounded-xl px-4 py-2 border border-white/20">
              <p className="font-bold text-lg">{ART_G6_S2_UNITS.reduce((s, u) => s + u.lessons.length, 0)}</p>
              <p className="text-white/70">دروس</p>
            </div>
            <div className="bg-white/10 rounded-xl px-4 py-2 border border-white/20">
              <p className="font-bold text-lg">{ART_G6_S2_TOTAL}</p>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-4">
        {ART_G6_S2_UNITS.map((unit, i) => (
          <motion.button key={unit.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            transition={{ delay: i * 0.1 }} onClick={() => setActiveUnit(i)}
            className="w-full bg-white rounded-2xl shadow-sm border border-emerald-100 overflow-hidden hover:shadow-md hover:border-emerald-300 transition-all text-right">

...
import { ArrowLeft, ArrowRight, CheckCircle2, Crown, BookOpen, Music, Headphones, Sparkles, ArrowRight as BackIcon } from 'lucide-react';
...
import { ENGLISH_G1_S2_UNITS as ENGLISH_UNITS, getExercisesForEnglishG1S2Lesson as getExercisesForLetterEn, TOTAL_QUESTIONS_ENGLISH_G1_S2 as TOTAL_QUESTIONS_EN, TOTAL_LESSONS_ENGLISH_G1_S2 as TOTAL_LESSONS_EN, WORD_EMOJI_EN } from '@/lib/englishGrade1S2';
import { withQuestionImages } from '@/lib/bankImagePools';

// Adapter: wrap unit vocab as lessons for the existing runner
const getAdaptiveLadder = () => [];
// Map ENGLISH_UNITS to the lesson format expected by the runner
const ADAPTED_UNITS = ENGLISH_UNITS.map(u => ({
  ...u,
  lessons: u.vocab.map(v => ({ letter: v.emoji, words: [v.word], title_ar: v.word })),
}));

const EXERCISES_PER_LESSON = 12;
...
// ألوان الوحدات الأربع — عائلتي / في المدرسة / جسمي وطعامي / الحيوانات والطبيعة
const UNIT_THEME = [
...
const BANNER = "https://media.base44.com/images/public/6a459ddfe6fb4264c098136d/d7d22d825_generated_image.png";
...
export default function EnglishGrade1S2() {
  ...
  const exercises = useMemo(
    () => (active ? withQuestionImages(getExercisesForLetterEn(active.unitIdx, active.lessonIdx), 'english') : []),
    [active]
  );
  ...
  if (active && !done && exercise) {
    ...
    return (
      <div className="space-y-6" dir="rtl">
        <PageHeader
          title={`درس الحرف (${lesson.letter}) — وحدة ${unit.title_ar}`}
          subtitle={`تمرين ${toArabicNumber(exIdx + 1)} من ${toArabicNumber(exercises.length)}`}
          breadcrumbs={[{ label: t('home'), path: '/' }, { label: 'اللغة الإنجليزية — الصف الأول ف٢', path: '/english-grade1-s2' }, { label: `الحرف ${lesson.letter}` }]}
        />
        ...
        <div className="max-w-2xl mx-auto space-y-5">
          ...
          <div className="flex items-center gap-3">
            <div className={`w-12 h-12 rounded-xl ${th.tile} flex items-center justify-center flex-shrink-0`}>
              <span className={`text-2xl font-heading font-bold ${th.tileText}`}>{lesson.letter}</span>
            </div>
    ...
    return (
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-xl mx-auto space-y-5 pt-8" dir="rtl">
        <div className="rounded-2xl p-8 text-center bg-gradient-to-br from-[#263D84] to-[#3E57A4] text-white">
          ...
          <div className="mt-3 text-sm text-white/70">درس الحرف ({lesson.letter}) — وحدة {unit.title_ar}</div>
        </div>
  ...
  const renderLessonTile = (l, li, th, onClick) => (
    <button onClick={onClick} className={`group flex flex-col items-center gap-2 p-3 rounded-2xl border-2 ${th.tileBorder} ${th.hover} hover:shadow-md transition-all hover:-translate-y-0.5`}>
      <div className={`w-16 h-16 rounded-2xl ${th.tile} flex items-center justify-center group-hover:scale-110 transition-transform`}>
        <span className={`text-3xl font-heading font-bold ${th.tileText}`}>{l.letter}</span>
      </div>
  ...
  return (
    <InteractiveBankBrowser
      ...
      banner={BANNER}
      title="English We Can! 1 — الصف الأول الابتدائي · ف٢"
      subtitle={`Toys • Food • Animals • Days & Weather • Feelings (${toArabicNumber(ADAPTED_UNITS.length)} وحدات · ${toArabicNumber(TOTAL_LESSONS_EN)} درسًا · ${toArabicNumber(TOTAL_QUESTIONS_EN)} سؤالًا)`}
      bankLabel="اللغة الإنجليزية — الصف الأول ف٢"
      homeLabel={t('home')}

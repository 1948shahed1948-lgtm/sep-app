...
import { TOPGOAL2_UNITS, TOPGOAL2_TOTAL } from '@/lib/englishTopGoal2Bank';
    ...
    <div className="space-y-4" dir="ltr">
      ...
      <div className="grid grid-cols-2 gap-3">
        {opts.map((val, i) => {
          ...
          return (
            <button key={i} disabled={answered} onClick={() => onAnswer(val)} className={cls}>
              {labels[i]}
            </button>
          );
        })}
    ...
    <div className="space-y-4" dir="ltr">
      ...
      <div className="grid grid-cols-1 gap-2">
        {q.options.map((opt, i) => {
          ...
          let cls = 'border-2 rounded-xl px-4 py-3 text-left text-sm font-medium transition-all ';
          if (!answered) cls += 'border-slate-200 hover:border-blue-400 hover:bg-blue-50 cursor-pointer';
          else if (isCorrect) cls += 'border-emerald-400 bg-emerald-50 text-emerald-800';
          else if (isPicked) cls += 'border-rose-400 bg-rose-50 text-rose-800';
          else cls += 'border-slate-200 opacity-50';
          return (
            <button key={i} disabled={answered} onClick={() => onAnswer(i)} className={cls}>
              <span className="font-bold text-slate-400 mr-2">{i + 1}.</span> {opt}
            </button>
          );
        })}
      </div>
...
function OrderingQ({ q, onAnswer, answered }) {
  ...
  const check = () => {
    ...
    const isCorrect = items.every((it, i) => it === correct[i]);
    setChecked(true);
  ...
  return (
    <div className="space-y-3" dir="ltr">
      ...
      <p className="text-xs text-slate-400">💡 Use the arrows to arrange in the correct order</p>
      ...
      {!checked && !answered && <Button onClick={check} className="w-full">Check Order</Button>}
    </div>
  );
}
...
function MatchingQ({ q, onAnswer, answered }) {
  ...
  const check = () => {
    const correct = q.pairs.every((p) => connections[p.left] === p.right);
    setChecked(true);
  ...
  return (
    <div className="space-y-3" dir="ltr">
      <p className="text-sm font-bold text-slate-700">{q.q}</p>
      <p className="text-xs text-slate-400">💡 Click a left item, then click its match on the right</p>
      <div className="grid grid-cols-2 gap-2">
        <div className="space-y-2">
          {leftItems.map((l) => {
            const isSelected = selected?.val === l;
            const isConnected = !!connections[l];
            return (
              <button key={l} onClick={() => handleLeft(l)} disabled={checked}
                className={`w-full text-xs px-2 py-2 rounded-lg border-2 text-left transition-all ${isSelected ? 'border-blue-500 bg-blue-50' : isConnected ? 'border-emerald-400 bg-emerald-50' : 'border-slate-200 hover:border-blue-300'}`}>
                {l} {isConnected && <span className="text-emerald-600 text-[10px]">→ {connections[l]}</span>}
              </button>
            );
          })}
        </div>
        <div className="space-y-2">
          {shuffledRight.map((r) => (
            <button key={r} onClick={() => handleRight(r)} disabled={checked}
              className={`w-full text-xs px-2 py-2 rounded-lg border-2 text-left transition-all ${selected?.side === 'left' ? 'border-amber-400 bg-amber-50 cursor-pointer' : 'border-slate-200'}`}>
              {r}
            </button>
          ))}
        </div>
      ...
      {!checked && !answered && Object.keys(connections).length === leftItems.length && (
        <Button onClick={check} className="w-full">Check Answers</Button>
      )}
    </div>
  );
}
...
function ClassifyQ({ q, onAnswer, answered }) {
  ...
  return (
    <div className="space-y-3" dir="ltr">
      <p className="text-sm font-bold text-slate-700">{q.q}</p>
      <p className="text-xs text-slate-400">💡 Click an item and hover to pick its category</p>
      {pool.length > 0 && (
        <div className="flex flex-wrap gap-2 p-3 bg-slate-50 rounded-xl border border-dashed border-slate-300">
          {pool.map((item) => (
            <div key={item.text} className="relative group">
              ...
              <div className="absolute top-7 left-0 z-10 hidden group-hover:flex flex-col gap-1 bg-white border border-slate-200 rounded-lg shadow-lg p-1 w-40">
                {q.categories.map((cat) => (
                  <button key={cat} onClick={() => assign(item, cat)} className="text-xs px-2 py-1 rounded hover:bg-blue-50 text-left">{cat}</button>
                ))}
              </div>
            </div>
          ))}
      ...
      <div className="grid grid-cols-2 gap-3">
        {q.categories.map((cat, ci) => (
          <div key={cat} className="bg-slate-50 rounded-xl border-2 border-dashed border-slate-300 p-2 min-h-[80px]">
      ...
      {!checked && !answered && pool.length === 0 && (
        <Button onClick={check} className="w-full">Check Classification</Button>
      )}
    </div>
  );
}
...
function WordOrderQ({ q, onAnswer, answered }) {
  ...
  const check = () => {
    const sentence = slots.join(' ');
    const isCorrect = sentence === q.correct_sentence || sentence === q.correct_sentence.replace(/\./g, '');
    setChecked(true);
    onAnswer(isCorrect ? '1' : '0');
  };
  ...
  return (
    <div className="space-y-3" dir="ltr">
      <p className="text-sm font-bold text-slate-700">{q.q}</p>
      <p className="text-xs text-slate-400">💡 Click the words in the correct order to form the sentence</p>
      <div className="min-h-[48px] p-3 bg-slate-50 border-2 border-dashed border-slate-300 rounded-xl flex flex-wrap gap-2">
        ...
        {slots.length === 0 && <span className="text-xs text-slate-400">Click words below...</span>}
      </div>
      ...
      <div className="flex gap-2">
        {!checked && pool.length === 0 && <Button onClick={check} className="flex-1">Check Sentence</Button>}
        {!checked && <Button onClick={reset} variant="outline" className="gap-1"><RotateCcw className="w-3 h-3" />Reset</Button>}
      </div>
      {checked && (
        <p className="text-xs text-slate-500 bg-slate-50 rounded-lg p-2">
          Correct sentence: <span className="font-bold text-emerald-700">{q.correct_sentence}</span>
        </p>
      )}
    </div>
  );
...
const TYPE_LABEL = {
  yesno: { label: 'True / False', color: 'bg-emerald-100 text-emerald-700' },
  mc: { label: 'Multiple Choice', color: 'bg-blue-100 text-blue-700' },
  ordering: { label: 'Ordering', color: 'bg-amber-100 text-amber-700' },
  matching: { label: 'Matching', color: 'bg-violet-100 text-violet-700' },
  classify: { label: 'Classify', color: 'bg-rose-100 text-rose-700' },
  word_order: { label: 'Word Order', color: 'bg-indigo-100 text-indigo-700' },
};

// ─── Main Page ───────────────────────────────────────────────
export default function EnglishTopGoal2() {
  ...
  const unit = active ? TOPGOAL2_UNITS[active.unitIdx] : null;
  ...
  if (active && !done && question) {
    ...
    const badge = TYPE_LABEL[question.type] || { label: 'Question', color: 'bg-slate-100 text-slate-700' };
    ...
    return (
      <div className="space-y-6">
        <PageHeader
          ...
          breadcrumbs={[
            { label: t('home'), path: '/' },
            { label: 'English TopGoal 2', path: '/english-topgoal2' },
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div className="flex items-center gap-2 pb-2 border-b border-slate-100">
                ...
                <span className="text-xs text-slate-400">TopGoal 2 — English — Grade 5 — Semester 2</span>
              </div>
            ...
            <>
              {correct ? (
                ...
                <WrongAnswerTeacher
                  ...
                  goldenRule={question.golden_rule}
                  lang="en"
                  isRTL={false}
                />
              )}
              <Button onClick={next} className="w-full gap-2 py-6 text-base font-bold">
                {qIdx < questions.length - 1 ? 'Next Question' : 'Finish Lesson'}
  ...
  if (active && done) {
    ...
    return (
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        className="max-w-xl mx-auto space-y-5 pt-8 px-4">
        <div className="rounded-2xl p-8 text-center bg-gradient-to-br from-[#263D84] to-[#3E57A4] text-white">
          ...
          <h2 className="text-3xl font-heading font-bold mb-1">{score} / {questions.length}</h2>
          <p className="text-white/80">
            {pct >= 80 ? 'Excellent mastery! 🏆' : pct >= 50 ? 'Good job, keep reviewing! 💪' : 'Needs review — try again! 📖'}
          </p>
          <p className="text-white/60 text-sm mt-2">{lesson.title_ar} — {unit.title_ar}</p>
        </div>
        <div className="flex gap-3">
          <Button onClick={() => openLesson(active.unitIdx, active.lessonIdx)} variant="outline" className="flex-1">
            Retry Lesson
          </Button>
          <Button onClick={backToUnits} className="flex-1 gap-2">
            Other Units <Arrow className="w-4 h-4" />
          </Button>
        </div>
      </motion.div>
    );
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        title="English — TopGoal 2 · Grade 5 · Semester 2"
        subtitle={`${TOPGOAL2_UNITS.length} Units · ${TOPGOAL2_TOTAL} Questions — Personal Interests · Storylines · Outdoor Activities · Outfits`}
        breadcrumbs={[{ label: t('home'), path: '/' }, { label: 'English — TopGoal 2' }]}
      />

      <div className="max-w-4xl mx-auto px-4 space-y-6">
        <div className="grid grid-cols-3 gap-3">
          {[
            { n: TOPGOAL2_UNITS.length, l: 'Units' },
            { n: TOPGOAL2_UNITS.reduce((s, u) => s + u.lessons.length, 0), l: 'Lessons' },
            { n: TOPGOAL2_TOTAL, l: 'Questions' },
          ].map((s) => (
          ...
          <div key={unit.id} className="space-y-4">
            <div className={`rounded-2xl bg-gradient-to-r ${unit.color} p-4 flex items-center gap-3`}>
              ...
              <div>
                ...
                <p className="text-white/70 text-xs">{unit.lessons.reduce((s, l) => s + l.questions.length, 0)} questions</p>
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {unit.lessons.map((lesson, li) => (
                <button
                  ...
                  className="group bg-white rounded-2xl border-2 border-slate-100 hover:border-blue-300 hover:shadow-md transition-all p-4 text-left flex items-center gap-3"
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">{lesson.questions.length} questions</p>

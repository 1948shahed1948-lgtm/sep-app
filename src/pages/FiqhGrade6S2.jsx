...
import { ChevronRight, ChevronLeft, RotateCcw, BookOpen, CheckCircle2, XCircle } from 'lucide-react';
import { fiqhGrade6S2Units, FIQH_GRADE6_S2_TOTAL } from '@/lib/fiqhGrade6S2Bank';
    ...
    <div className="space-y-3 mt-4" dir="rtl">
      <div className={`rounded-xl p-4 border-2 ${isCorrect ? 'bg-emerald-50 border-emerald-200' : 'bg-red-50 border-red-200'}`}>
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="font-bold text-sm">{isCorrect ? '🎉 ممتاز!' : '📚 شرح المعلم'}</span>
        </div>
      ...
      {q.feedback_question && (
        <div className="rounded-xl p-3 bg-indigo-50 border border-indigo-200">
          ...
          <details><summary className="text-xs text-indigo-600 cursor-pointer font-medium">عرض الإجابة</summary>
...
function MCQQuestion({ q, onAnswer }) {
  ...
  const submit = (i) => { if (picked !== null) return; setPicked(i); setTimeout(() => onAnswer(i === correctIdx, i, correctIdx), 100); };
  return (
...
function LessonRunner({ lesson, unit, onBack }) {
  ...
  const handleAnswer = (correct) => { if (answered) return; setAnswered(true); if (correct) setScore(s => s + 1); };
  const next = () => { if (idx + 1 < questions.length) { setIdx(i => i + 1); setAnswered(false); setKey(k => k + 1); } else setDone(true); };
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        await base44.entities.QuizResult.create({ user_id: user.id, quiz_type: 'lesson_quiz', total_questions: questions.length, correct_count: score, wrong_count: questions.length - score, score_percentage: Math.round((score / questions.length) * 100), errors_detail: [], weakness_detected: [] });
        await base44.entities.StudentActivity.create({ user_id: user.id, activity_type: 'practice', label_ar: `${unit.title_ar} — ${lesson.title_ar}`, label_en: lesson.title_ar, details: `${score}/${questions.length}`, route: '/fiqh-grade6-s2' });
      } catch (e) { /* silent */ }
    })();
  }, [done]);
    ...
    return (
      <div className="text-center space-y-6 py-8" dir="rtl">
        ...
        <div className="flex gap-3 justify-center">
          <button onClick={() => { setIdx(0); setScore(0); setDone(false); setAnswered(false); setKey(k => k + 1); }} className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-primary text-white font-bold text-sm"><RotateCcw className="w-4 h-4" /> إعادة</button>
  ...
  return (
    <div className="space-y-4">
      <button onClick={onBack} className="flex items-center gap-2 text-sm text-slate-500 hover:text-primary font-medium" dir="rtl"><ChevronRight className="w-4 h-4" /> {lesson.title_ar}</button>
      <div className="flex items-center justify-between" dir="rtl">
        <span className="text-xs text-slate-500 font-medium">السؤال {idx + 1} / {questions.length}</span>
        <div className="w-32 h-2 bg-slate-200 rounded-full overflow-hidden"><div className="h-full bg-primary rounded-full transition-all" style={{ width: `${((idx + 1) / questions.length) * 100}%` }} /></div>
      </div>
      <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
        <AnimatePresence mode="wait">
          <motion.div key={key} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }}>
            <QuestionRenderer q={questions[idx]} onAnswer={handleAnswer} />
          </motion.div>
        </AnimatePresence>
      </div>
      {answered && (
        <motion.button initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} onClick={next} className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-primary text-white font-bold text-sm shadow-lg">
          {idx + 1 < questions.length ? 'السؤال التالي' : 'عرض النتيجة'}<ChevronLeft className="w-4 h-4" />
        </motion.button>
      )}
    </div>
  );
...
export default function FiqhGrade6S2() {
  const [selectedUnit, setSelectedUnit] = useState(null);
  const [selectedLesson, setSelectedLesson] = useState(null);

  if (selectedUnit && selectedLesson) return <div className="max-w-2xl mx-auto p-4"><LessonRunner lesson={selectedLesson} unit={selectedUnit} onBack={() => setSelectedLesson(null)} /></div>;

  if (selectedUnit) return (
    <div className="max-w-2xl mx-auto p-4 space-y-4" dir="rtl">
      <button onClick={() => setSelectedUnit(null)} className="flex items-center gap-2 text-sm text-slate-500 hover:text-primary font-medium"><ChevronRight className="w-4 h-4" /> العودة للوحدات</button>
      ...
      <div className="space-y-3">
        {selectedUnit.lessons.map((lesson, i) => (
          <motion.button key={lesson.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }} onClick={() => setSelectedLesson(lesson)} className="w-full text-right p-4 rounded-xl bg-white border-2 border-slate-200 hover:border-primary/40 hover:shadow-md transition-all">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0" style={{ backgroundColor: selectedUnit.color }}>{i + 1}</div>
              <div>
                <p className="font-bold text-slate-800 text-sm">{lesson.title_ar}</p>
                <p className="text-xs text-slate-400 mt-0.5">{lesson.questions.length} أسئلة</p>
              </div>
            </div>
          </motion.button>
        ))}
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 space-y-6" dir="rtl">
      <div className="relative overflow-hidden rounded-2xl px-6 py-8 text-white" style={{ background: 'linear-gradient(135deg, #1e3a5f 0%, #0f2845 100%)' }}>
        <div className="relative">
          ...
          <div className="flex gap-4 text-center mt-4">
            <div className="bg-white/10 rounded-xl px-4 py-2"><p className="text-xl font-bold">{fiqhGrade6S2Units.length}</p><p className="text-xs text-white/70">وحدات</p></div>
            <div className="bg-white/10 rounded-xl px-4 py-2"><p className="text-xl font-bold">{fiqhGrade6S2Units.reduce((t, u) => t + u.lessons.length, 0)}</p><p className="text-xs text-white/70">دروس</p></div>
            <div className="bg-white/10 rounded-xl px-4 py-2"><p className="text-xl font-bold">{FIQH_GRADE6_S2_TOTAL}</p><p className="text-xs text-white/70">سؤالاً</p></div>
          </div>
        </div>
      </div>
      <div className="grid sm:grid-cols-2 gap-4">
        {fiqhGrade6S2Units.map((unit, i) => (
          <motion.button key={unit.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} onClick={() => setSelectedUnit(unit)} className="text-right p-5 rounded-2xl text-white shadow-lg hover:-translate-y-1 transition-all duration-300 hover:shadow-xl" style={{ background: `linear-gradient(135deg, ${unit.color}cc, ${unit.color})` }}>
            <div className="text-3xl mb-2">{unit.icon}</div>
            <h3 className="font-bold text-base mb-1">{unit.title_ar}</h3>
            <div className="flex items-center gap-2 text-white/70 text-xs"><BookOpen className="w-3 h-3" /><span>{unit.lessons.length} دروس · {unit.lessons.reduce((s, l) => s + l.questions.length, 0)} سؤال</span></div>
          </motion.button>
        ))}
      </div>
      <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4">
        <h3 className="font-bold text-emerald-800 text-sm mb-2">📋 محاور الفصل الثاني — الفقه</h3>
        <div className="grid grid-cols-2 gap-2 text-xs text-emerald-700">
          {['الزكاة (مكانتها وشروطها)', 'زكاة الفطر', 'صدقة التطوع والصدقة الجارية', 'الصيام (مكانته وحكمه)', 'مفسدات الصوم وآدابه', 'العشر الأواخر وليلة القدر', 'الحج والعمرة (الحكم والشروط)', 'المواقيت المكانية', 'الإحرام وأركان العمرة'].map((item, i) => (
            <div key={i} className="flex items-center gap-1"><span>☑</span> {item}</div>
          ))}
        </div>
      </div>
    </div>

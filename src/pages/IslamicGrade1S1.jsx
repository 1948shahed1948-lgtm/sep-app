...
import { ArrowLeft, ArrowRight, Printer, Moon, Sun, Award, RotateCcw, Filter, X, ChevronDown } from 'lucide-react';
...
import {
  ISLAMIC_S1_UNITS, ISLAMIC_S1_QUESTIONS,
  getQuestionsForLesson,
  TOTAL_LESSONS_IS1, TOTAL_QUESTIONS_IS1,
  BLOOM_LABELS, DIFFICULTY_LABELS, TYPE_LABELS,
} from '@/lib/islamicGrade1S1Bank';

const STORAGE_KEY = 'islamic1s1_progress_v1';
const THEME_KEY = 'islamic1s1_theme';
      ...
      <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="bg-white rounded-3xl p-6 max-w-lg w-full max-h-[85vh] overflow-y-auto shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div className="rounded-xl bg-amber-50 border border-amber-200 p-3 text-center">
            <div className="text-2xl font-bold text-amber-700">{toAr(totalPoints)}</div><div className="text-xs text-slate-500">نقاط متراكمة</div>
          </div>
          <div className="rounded-xl bg-emerald-50 border border-emerald-200 p-3 text-center">
            <div className="text-2xl font-bold text-emerald-700">{toAr(masteredCount)}</div><div className="text-xs text-slate-500">دروس متقنة ✓</div>
          </div>
        </div>
        <div className="space-y-2 mb-4">
          {ISLAMIC_S1_UNITS.map((u) => (
        ...
        <div className="rounded-xl bg-rose-50 border border-rose-200 p-3">
          ...
          {mistakes.length === 0 ? <p className="text-xs text-slate-500">لا أخطاء 👏</p> :
...
function PrintDialog({ onClose }) {
  const [units, setUnits] = useState(ISLAMIC_S1_UNITS.map(() => true));
  const [withExplain, setWithExplain] = useState(true);
  const [diff, setDiff] = useState({ easy: true, medium: true, hard: true });

  const doPrint = () => {
    const selected = ISLAMIC_S1_UNITS.filter((_, i) => units[i]);
    const qs = ISLAMIC_S1_QUESTIONS.filter((q) => selected.some((u) => u.id === q.unit) && diff[q.difficulty]);
    const rows = qs.map((q, i) => {
      ...
      let explain = '';
      if (withExplain && q.explanation) explain += `<div style="margin-top:6px;padding:6px;background:#f0fdf4;border-right:3px solid #059669;font-size:12px"><b>الشرح:</b> ${q.explanation}</div>`;
      if (withExplain && q.goldenRule) explain += `<div style="margin-top:4px;padding:6px;background:#fffbeb;border:2px solid #fbbf24;font-size:12px"><b>👑 القاعدة الذهبية:</b> ${q.goldenRule}</div>`;
      if (q.paragraph) explain += `<div style="margin-top:4px;padding:6px;background:#fef2f2;color:#dc2626;font-weight:bold;font-size:11px">📖 ${q.paragraph} (ص ${q.page})</div>`;
      return `<div style="page-break-inside:avoid;margin-bottom:12px;padding:10px;border:1px solid #e2e8f0;border-radius:8px">
        <div style="font-weight:bold;margin-bottom:6px">${i + 1}. ${q.question}</div>
        <ul style="list-style:none;padding:0">${opts_html}</ul>${explain}</div>`;
    }).join('');
    const html = `<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="utf-8"><title>التربية الإسلامية — الصف الأول ف١</title>
    <style>body{font-family:'Tajawal',sans-serif;padding:20px;max-width:800px;margin:auto}h1{text-align:center;color:#0a1628}@media print{body{padding:10px}}</style></head>
    <body><h1>بنك أسئلة التربية الإسلامية — الصف الأول الابتدائي (ف١)</h1><div style="text-align:center;font-size:22px;color:#c9a227">⭐⭐⭐</div>${rows}
    <div style="margin-top:20px;text-align:center;font-size:11px;color:#64748b;border-top:1px solid #e2e8f0;padding-top:8px">أعدّ هذا البنك فريق المنصة التعليمية — ${qs.length} سؤالاً</div></body></html>`;
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-50 bg-slate-900/70 flex items-center justify-center p-4" onClick={onClose}>
      <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2"><Printer className="w-5 h-5" /> طباعة PDF</h3>
          <button onClick={onClose}><X className="w-5 h-5 text-slate-400" /></button>
        </div>
        <div className="space-y-3 text-sm">
          <div>
            <div className="font-bold text-slate-700 mb-1">الوحدات:</div>
            {ISLAMIC_S1_UNITS.map((u, i) => (
        ...
        <button onClick={doPrint} className="w-full mt-4 py-3 rounded-xl bg-[#0a1628] text-white font-bold flex items-center justify-center gap-2">
  ...
  if (done) {
    ...
    return (
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-xl mx-auto space-y-5 pt-6" dir="rtl">
        <div className="rounded-2xl p-8 text-center text-white" style={{ background: 'linear-gradient(135deg, #0a1628 0%, #1e3a5f 100%)' }}>
          <div className="text-4xl mb-2">{pct >= 80 ? '🏆' : pct >= 50 ? '💪' : '📖'}</div>
          <h2 className="text-3xl font-bold mb-1">{toAr(score)} / {toAr(questions.length)}</h2>
          <p className="text-white/80">{pct >= 80 ? 'ممتاز!' : pct >= 50 ? 'جيد، واصل' : 'راجع الدرس'}</p>
          <div className="mt-1 text-sm text-white/70">{lesson.title_ar}</div>
        </div>
        <div className="flex gap-3">
        ...
        <button onClick={onBack} className="w-full py-3 rounded-xl text-slate-500">العودة</button>
      </motion.div>
    );
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4" dir="rtl">
      ...
      <button onClick={onBack} className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-800">
        <ArrowRight className="w-4 h-4 rotate-180" /> عودة
      </button>
      ...
      <div className="relative rounded-2xl bg-white border-2 border-amber-200 shadow-md p-5 overflow-hidden">
        ...
        <div className="flex items-center gap-2 mb-3 pr-12 flex-wrap">
          ...
          <span className="text-xs px-2.5 py-1 rounded-full bg-amber-100 text-amber-700">{BLOOM_LABELS[q.bloom]}</span>
          <span className="text-xs px-2.5 py-1 rounded-full bg-indigo-100 text-indigo-700">{DIFFICULTY_LABELS[q.difficulty]}</span>
      ...
      {answered && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
          ...
          <button onClick={next} className="w-full py-3.5 rounded-xl bg-[#0a1628] text-white font-bold flex items-center justify-center gap-2">
            {idx < questions.length - 1 ? 'السؤال التالي' : 'إنهاء الدرس'} <ArrowLeft className="w-5 h-5" />
          </button>
        </motion.div>
      )}
    </div>
...
export default function IslamicGrade1S1() {
  const { t } = useLang();
  ...
  const nextLesson = () => {
    ...
    const unit = ISLAMIC_S1_UNITS[unitIdx];
    if (lessonIdx < unit.lessons.length - 1) openLesson(unitIdx, lessonIdx + 1);
    else if (unitIdx < ISLAMIC_S1_UNITS.length - 1) openLesson(unitIdx + 1, 0);
    else setActiveLesson(null);
  };

  if (activeLesson) {
    const unit = ISLAMIC_S1_UNITS[activeLesson.unitIdx];
    ...
    return (
      <div className={dark ? 'dark' : ''}>
        <div className="min-h-screen bg-[#faf8f0] dark:bg-slate-900">
          <PageHeader
            title={`${lesson.title_ar} — ${unit.title_ar}`}
            subtitle="التربية الإسلامية · الصف الأول · ف١"
            breadcrumbs={[{ label: t('home'), path: '/' }, { label: 'الإسلامية ف١', path: '/islamic-grade1-s1' }, { label: lesson.title_ar }]}
          />
  ...
  return (
    <div className={dark ? 'dark' : ''}>
      <div className="min-h-screen bg-[#faf8f0] dark:bg-slate-900" dir="rtl">
        <PageHeader
          title="التربية الإسلامية — الصف الأول الابتدائي · ف١"
          subtitle={`${toAr(ISLAMIC_S1_UNITS.length)} وحدات · ${toAr(TOTAL_LESSONS_IS1)} دروسًا · ${toAr(TOTAL_QUESTIONS_IS1)} سؤالاً`}
          breadcrumbs={[{ label: t('home'), path: '/' }, { label: 'التربية الإسلامية — الصف الأول ف١' }]}
        />
        <div className="max-w-5xl mx-auto px-4 pb-12 space-y-5">
          {/* أدوات */}
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex gap-2">
              <button onClick={() => setShowFile(true)} className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-white border border-slate-200 text-sm font-medium text-slate-700 hover:shadow-sm">
                <Award className="w-4 h-4 text-amber-500" /> ملف الطالب
              </button>
              <button onClick={() => setShowPrint(true)} className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-white border border-slate-200 text-sm font-medium text-slate-700 hover:shadow-sm">
                <Printer className="w-4 h-4" /> طباعة
              </button>
            </div>
            <button onClick={() => setDark(!dark)} className="w-10 h-10 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-slate-600">
              {dark ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
            </button>
          </div>

          {/* الغلاف */}
          <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="relative overflow-hidden rounded-3xl p-8 text-center shadow-xl" style={{ background: 'linear-gradient(135deg, #0a1628 0%, #1e3a5f 60%, #2d5a9e 100%)' }}>
            <div className="absolute top-3 right-4 text-amber-300/30 text-3xl select-none">✦</div>
            <div className="absolute bottom-3 left-4 text-amber-300/30 text-3xl select-none">✦</div>
            <div className="relative z-10 space-y-2 text-white">
              <div className="flex justify-center"><GoldenSeal size="lg" /></div>
              <h1 className="text-2xl sm:text-3xl font-bold">بنك أسئلة التربية الإسلامية</h1>
              <p className="text-white/80">الصف الأول الابتدائي · الفصل الأول</p>
              <div className="text-amber-300 text-xl">⭐⭐⭐</div>
              <div className="flex items-center justify-center gap-4 text-xs text-white/60">
                <span>🌙 التوحيد</span><span>📖 القرآن</span><span>⭐ السيرة</span><span>🤝 السلام</span>
              </div>
            </div>
          </motion.section>
          ...
          <div className="rounded-2xl bg-white border border-slate-200 p-4">
            <div className="flex items-center gap-2 text-sm font-bold text-slate-700 mb-2"><Filter className="w-4 h-4" /> فلتر المستوى</div>
            <div className="flex gap-2 flex-wrap">
              {[['all', 'الكل'], ['easy', 'سهل'], ['medium', 'متوسط'], ['hard', 'صعب']].map(([k, v]) => (
                <button key={k} onClick={() => setFilterDiff(k)} className={`px-3 py-1.5 rounded-full text-xs font-medium ${filterDiff === k ? 'bg-[#0a1628] text-white' : 'bg-slate-100 text-slate-600'}`}>{v}</button>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-4">
            {ISLAMIC_S1_UNITS.map((u, ui) => (
              <motion.div key={u.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: ui * 0.08 }} className="rounded-2xl bg-white border-2 border-amber-100 shadow-sm overflow-hidden">
                <button onClick={() => setOpenUnit(openUnit === ui ? null : ui)} className="w-full flex items-center gap-3 p-4 hover:bg-amber-50/50 transition-colors">
                  <div className="w-14 h-14 rounded-xl flex items-center justify-center text-3xl flex-shrink-0 bg-[#0a1628]">{u.icon}</div>
                  ...
                  <ChevronDown className={`w-5 h-5 text-slate-400 transition-transform ${openUnit === ui ? 'rotate-180' : ''}`} />
                </button>
                <AnimatePresence>
                  {openUnit === ui && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="border-t border-amber-100">
                      <div className="grid sm:grid-cols-2 gap-3 p-4">
                        {u.lessons.map((l, li) => {
                          ...
                          const soon = cnt === 0;
                          return (
                            <button key={l.id} disabled={soon} onClick={() => !soon && openLesson(ui, li)}
                              className={`group flex flex-col gap-2 p-4 rounded-xl border-2 text-right transition-all ${soon ? 'border-slate-100 opacity-60 cursor-not-allowed' : 'border-slate-100 hover:border-amber-400 hover:shadow-md hover:-translate-y-0.5'}`}>
                              <div className="text-xs font-bold text-amber-600">الدرس {toAr(li + 1)}</div>
                              <div className="text-sm font-medium text-slate-700">{l.title_ar}</div>
                              {soon ? <div className="text-xs text-amber-600 font-bold">قريباً</div> : <div className="text-xs text-slate-400">{toAr(cnt)} سؤال</div>}
                              {st && !soon && <div className="text-xs font-bold">{st.mastered ? '✓ متقن' : `⚠️ ${toAr(st.bestPct)}٪`}</div>}
                            </button>
                          );
                        })}
                      </div>

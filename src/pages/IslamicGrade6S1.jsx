...
import { islamicGrade6S1Units, ISLAMIC_GRADE6_S1_TOTAL } from '@/lib/islamicGrade6S1Bank';
    ...
    <div className="space-y-3" dir="rtl">
      ...
      <div className="grid grid-cols-2 gap-3">
        {q.options.map((o, i) => {
          ...
          return (
            <button key={i} onClick={() => submit(i)} disabled={picked !== null}
              className={`w-full text-center px-4 py-4 rounded-xl font-bold text-sm transition-all ${cls}`}>
              {o.text}
            </button>
          );
        })}
...
export default function IslamicGrade6S1() {
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 space-y-6" dir="rtl">
        ...
        <div className="relative">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-white/70 text-sm">الصف السادس الابتدائي — الفصل الأول</p>
            </div>
          </div>
          <div className="flex gap-4 text-center mt-4">
            <div className="bg-white/10 rounded-xl px-4 py-2">
              <p className="text-xl font-bold">{islamicGrade6S1Units.length}</p>
              <p className="text-xs text-white/70">وحدات</p>
            </div>
            <div className="bg-white/10 rounded-xl px-4 py-2">
              <p className="text-xl font-bold">{islamicGrade6S1Units.reduce((t, u) => t + u.lessons.length, 0)}</p>
              <p className="text-xs text-white/70">دروس</p>
            </div>
            <div className="bg-white/10 rounded-xl px-4 py-2">
              <p className="text-xl font-bold">{ISLAMIC_GRADE6_S1_TOTAL}</p>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {islamicGrade6S1Units.map((unit, i) => (
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4" dir="rtl">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="font-bold text-amber-800 text-sm">محاور الكتاب</h3>
        </div>
        <div className="grid grid-cols-2 gap-2 text-xs text-amber-700">
          <div className="flex items-center gap-1"><span>☑</span> التوحيد — الإيمان</div>
          <div className="flex items-center gap-1"><span>☑</span> التوحيد — الشرك</div>
          <div className="flex items-center gap-1"><span>☑</span> البدع والذنوب والمعاصي</div>
          <div className="flex items-center gap-1"><span>☑</span> منهج أهل السنة والجماعة</div>
          <div className="flex items-center gap-1"><span>☑</span> هدي النبي ﷺ مع الصغار والأقارب</div>
          <div className="flex items-center gap-1"><span>☑</span> الفقه — صلاة الجمعة والعيدين</div>
        </div>
      </div>
    </div>
  );

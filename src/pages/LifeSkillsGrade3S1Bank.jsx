...
import {
  ...
  LS3_S1_QUESTIONS,
  TOTAL_QUESTIONS_LS3,
} from '@/lib/lifeSkillsGrade3S1Bank';
    ...
    <div className="space-y-2">
      ...
      {items.map((it, i) => (
        <div key={i} className="flex items-center gap-2">
          <span className="w-7 h-7 rounded-full bg-teal-100 text-teal-700 flex items-center justify-center font-bold text-xs flex-shrink-0">{toAr(i+1)}</span>
    ...
    <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {pool.map(it => (
              <span key={it.id} onClick={() => setSel(it)}
                className={`px-3 py-1.5 rounded-lg border cursor-pointer text-sm ${sel?.id === it.id ? 'border-teal-500 bg-teal-50 font-bold' : 'bg-white border-slate-300 hover:border-teal-400'}`}>
                {it.text}
              </span>
            ))}
          </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {q.categories.map((cat, ci) => (
          <div key={ci} onClick={() => moveTo(ci)} className="rounded-xl border-2 border-slate-200 p-3 min-h-24 cursor-pointer hover:border-teal-300 transition-colors">
...
function GoldenBox({ text }) {
  return (
    <div className="rounded-xl p-3 border-2 mt-3" style={{ borderColor: '#0891b2', background: 'linear-gradient(135deg,#f0f9ff,#e0f7fa)' }}>
      <div className="flex items-center gap-2 mb-1">
        <Crown className="w-4 h-4 text-teal-600" />
        <span className="font-bold text-teal-800 text-sm">القاعدة الذهبية</span>
      </div>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={idx} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}
          ...
          <div className="px-4 pt-4 pb-2 border-b border-slate-100 flex flex-wrap gap-1.5">
            ...
            <span className="text-xs px-2 py-0.5 rounded-full bg-teal-100 text-teal-700">{TYPE_LABEL[q.type]}</span>
          </div>
...
// ── Main Page ─────────────────────────────────────────────────────────────
export default function LifeSkillsGrade3S1Bank() {
  ...
  const lessonQuestions = useMemo(() => {
    ...
    return LS3_S1_QUESTIONS.filter(q => q.lesson === selectedLesson.id);
  }, [selectedLesson]);
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-5 space-y-5">
        ...
        <div className="rounded-2xl p-5 text-white" style={{ background: `linear-gradient(135deg,${selectedUnit.color},${selectedUnit.color}99)` }}>
          ...
          <p className="text-white/60 text-xs mt-1">{selectedUnit.lessons.length} دروس · {LS3_S1_QUESTIONS.filter(q => q.unit === selectedUnit.id).length} سؤال</p>
        </div>
        <div className="space-y-3">
          {selectedUnit.lessons.map(lesson => {
            const lQ = LS3_S1_QUESTIONS.filter(q => q.lesson === lesson.id);
            return (
              <button key={lesson.id} onClick={() => openLesson(selectedUnit, lesson)}
                className="w-full rounded-2xl p-4 bg-white border-2 border-slate-200 hover:border-teal-400 transition-all text-right flex items-center gap-3 hover:shadow-md">
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-5 space-y-6">
      <div className="relative rounded-3xl overflow-hidden p-6 text-center shadow-xl"
        style={{ background: 'linear-gradient(135deg,#0c4a6e,#0891b2,#059669)' }}>
        ...
        <h1 className="text-white font-heading font-bold text-xl mb-1">المهارات الحياتية والأسرية</h1>
        <p className="text-white/70 text-sm">{LS3_S1_META.grade} · {LS3_S1_META.semester}</p>
        <p className="text-white/40 text-xs mt-1">{LS3_S1_META.year}</p>
        <div className="mt-3 flex justify-center gap-3 flex-wrap">
          <div className="bg-white/10 px-3 py-1.5 rounded-full text-xs text-white/80">
            {toAr(LS3_S1_UNITS.length)} وحدات
          </div>
          <div className="bg-teal-400/20 px-3 py-1.5 rounded-full text-xs text-teal-200">
            {toAr(TOTAL_QUESTIONS_LS3)} سؤال
          </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {LS3_S1_UNITS.map((unit, ui) => {
          const unitQ = LS3_S1_QUESTIONS.filter(q => q.unit === unit.id);
          return (
                ...
                <div className="mt-2 flex flex-wrap gap-1">
                  {unit.lessons.map(l => (
                    <span key={l.id} className="text-[10px] bg-white/20 text-white px-2 py-0.5 rounded-full">
                      {l.title_ar.length > 15 ? l.title_ar.slice(0, 15) + '...' : l.title_ar}
                    </span>
                  ))}
                </div>
              </div>
      ...
      <div className="rounded-2xl bg-teal-50 border border-teal-200 p-4 text-center">
        <Star className="w-5 h-5 text-teal-500 mx-auto mb-2" />
        <p className="text-teal-800 text-sm font-medium">{LS3_S1_META.publisher}</p>
        <p className="text-teal-600 text-xs mt-1">جميع الأسئلة مستخرجة من الكتاب المدرسي المعتمد</p>
      </div>
    </div>
  );
}
...
import { LIFE_SKILLS_G5_S2_UNITS, LIFE_SKILLS_G5_TOTAL } from '@/lib/lifeSkillsGrade5S2Bank';
import WrongAnswerTeacher from '@/components/aptitude/WrongAnswerTeacher';

// ====== مكوّن عرض المعلومة (info) ======
function InfoCard({ q }) {
  return (
    <div className="rounded-xl bg-blue-50 border-2 border-blue-200 p-4 flex items-start gap-3">
      ...
      <div>
        <p className="text-xs font-bold text-blue-700 mb-1">معلومة</p>
        <p dir="rtl" className="text-sm text-blue-900 leading-relaxed">{q.question_ar}</p>
        {q.image_url && (
          <div className="mt-2 rounded-lg overflow-hidden border border-blue-100">
            <img src={q.image_url} alt="" className="w-full max-h-40 object-cover" />
          </div>
        )}
      </div>
    </div>
...
function QuestionRenderer({ q, onAnswer, isAnswered }) {
  ...
  const handleClassifyPick = (binIdx) => {
    ...
    if (newItems.length === 0) {
      let allCorrect = true;
      newBins.forEach((bin, catIdx) => bin.forEach(it => { if (it.correct_category !== catIdx) allCorrect = false; }));
      onAnswer(allCorrect);
    }
  };
  ...
  const handleMatchPick = (side, idx) => {
    ...
    if (Object.keys(newConns).length === q.pairs_ar.length) {
      onAnswer(Object.entries(newConns).every(([r, l]) => parseInt(r) === l));
    }
  };
  ...
  if (q.type === 'multiple_choice' || q.type === 'fill_blank') {
    const COLORS = ['bg-pink-50 border-pink-200 hover:border-pink-400', 'bg-cyan-50 border-cyan-200 hover:border-cyan-400', 'bg-emerald-50 border-emerald-200 hover:border-emerald-400', 'bg-amber-50 border-amber-200 hover:border-amber-400'];
    const ACTIVE = ['bg-pink-100 border-pink-500', 'bg-cyan-100 border-cyan-500', 'bg-emerald-100 border-emerald-500', 'bg-amber-100 border-amber-500'];
    return (
          ...
          return (
            <button key={i} disabled={showResult} onClick={() => handleMC(i)}
              className={`w-full text-right px-4 py-3 rounded-xl border-2 text-sm font-medium transition-all
                ...
                ${showResult && !isPicked && !isCorrect ? 'bg-white border-slate-200 text-slate-400 opacity-70' : ''}
              `}
            >
      ...
      <div className="space-y-4">
        {!done && (
          <div className="rounded-xl border-2 border-dashed border-amber-300 bg-amber-50 p-4 text-center">
            <p className="text-xs text-amber-600 mb-2 font-bold">صنِّف هذا العنصر:</p>
            ...
            <div className="flex flex-wrap gap-2 justify-center mt-3">
              {q.categories_ar.map((cat, i) => (
                <button key={i} onClick={() => handleClassifyPick(i)}
                  className={`px-4 py-2 rounded-lg text-white text-sm font-bold transition-colors ${i % 2 === 0 ? 'bg-emerald-500 hover:bg-emerald-600' : 'bg-blue-500 hover:bg-blue-600'}`}>
                  {cat}
                </button>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {bins && bins.map((bin, i) => (
            <div key={i} className={`rounded-xl border-2 p-3 min-h-[60px] ${i % 2 === 0 ? 'border-emerald-200 bg-emerald-50' : 'border-blue-200 bg-blue-50'}`}>
              <p className={`text-xs font-bold mb-2 ${i % 2 === 0 ? 'text-emerald-700' : 'text-blue-700'}`}>{q.categories_ar[i]}</p>
              <div className="flex flex-wrap gap-1">
                {bin.map((it, j) => (
                  <span key={j} className="text-xs bg-white border rounded-full px-2 py-0.5 text-slate-700">{it.text}</span>
                ))}
              </div>
            </div>
          ))}
  ...
  if (q.type === 'matching') {
    ...
    return (
      <div className="space-y-3">
        <p className="text-xs text-slate-500 text-center">اضغط من العمود الأيمن ثم من العمود الأيسر لتوصيلهما:</p>
      ...
      <div className="space-y-3">
        ...
        {orderItems.map((item, i) => (
          <div key={item} className="flex items-center gap-2 bg-white border-2 border-slate-200 rounded-xl px-3 py-2">
            <span className="w-6 h-6 rounded-full bg-slate-100 flex items-center justify-center text-xs font-bold text-slate-600 flex-shrink-0">{i + 1}</span>
        ...
        {!checked && <Button onClick={checkOrder} className="w-full bg-amber-600 hover:bg-amber-700">تحقق من الترتيب ✓</Button>}
      </div>
    );
  }
    ...
    return (
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center py-10 px-4 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          ...
          <Button onClick={onBack} className="bg-emerald-600 hover:bg-emerald-700 gap-2"><ArrowRight className="w-4 h-4" />العودة للدروس</Button>
        </div>
      </motion.div>
    );
  }
    ...
    <div ref={containerRef} className="space-y-4">
      <div className="flex items-center gap-3">
        <div className="flex-1 bg-slate-100 rounded-full h-2">
          <div className="bg-emerald-500 h-2 rounded-full transition-all duration-500" style={{ width: `${(idx / questions.length) * 100}%` }} />
        </div>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={q.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
          ...
          {isInfo ? (
            <>
              <InfoCard q={q} />
              <Button onClick={next} className="w-full bg-blue-600 hover:bg-blue-700">
            ...
            <>
              <div className="flex items-start gap-2">
                <span className="text-xs bg-emerald-100 text-emerald-700 rounded-full px-2 py-0.5 font-bold flex-shrink-0 mt-0.5">{idx + 1}</span>
                <p className="text-base font-bold text-slate-800 leading-relaxed">{q.question_ar}</p>
              </div>

              {q.image_url && (
                <div className="rounded-xl overflow-hidden border border-slate-100 bg-slate-50">
                  <img src={q.image_url} alt="" className="w-full max-h-44 object-contain" />
                </div>
              )}
...
// ====== الصفحة الرئيسية ======
export default function LifeSkillsGrade5S2() {
  const [activeUnit, setActiveUnit] = useState(null);
  const [activeLesson, setActiveLesson] = useState(null);

  if (activeUnit !== null && activeLesson !== null) {
    const unit = LIFE_SKILLS_G5_S2_UNITS[activeUnit];
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {unit.lessons.map((lesson, i) => (
              <motion.button key={lesson.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                transition={{ delay: i * 0.06 }} onClick={() => setActiveLesson(i)}
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-teal-50" dir="rtl">
      <div className="relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #059669 0%, #0d9488 60%, #065f46 100%)' }}>
        ...
        <div className="relative max-w-3xl mx-auto px-4 py-10 sm:py-14 text-center text-white">
          ...
          <p className="text-white/80 text-sm">الصف الخامس — الفصل الثاني | التواصل · بيئتي · ملبسي · غذائي</p>
          <div className="flex justify-center gap-4 mt-5 text-xs flex-wrap">
            <div className="bg-white/10 rounded-xl px-4 py-2">
              <p className="font-bold text-lg">{LIFE_SKILLS_G5_S2_UNITS.length}</p>
              <p className="text-white/70">وحدات</p>
            </div>
            <div className="bg-white/10 rounded-xl px-4 py-2">
              <p className="font-bold text-lg">{LIFE_SKILLS_G5_S2_UNITS.reduce((s, u) => s + u.lessons.length, 0)}</p>
              <p className="text-white/70">دروس</p>
            </div>
            <div className="bg-white/10 rounded-xl px-4 py-2">
              <p className="font-bold text-lg">{LIFE_SKILLS_G5_TOTAL}</p>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-4">
        {LIFE_SKILLS_G5_S2_UNITS.map((unit, i) => (
            ...
            <div className="flex items-center gap-4 p-5">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 mt-2">
                  <span className="text-xs bg-emerald-100 text-emerald-700 rounded-full px-2 py-0.5">{unit.lessons.length} دروس</span>
                  <span className="text-xs bg-teal-100 text-teal-700 rounded-full px-2 py-0.5">{unit.lessons.reduce((s, l) => s + l.count, 0)} سؤالاً</span>
                </div>
              </div>

...
import { LUGHATI3_S2_UNITS, TOTAL_L3S2_QUESTIONS } from '@/lib/lughatiGrade3S2Bank';
...
// ─── الصفحة الرئيسية ───────────────────────────────────────
export default function LughatiGrade3S2() {
  ...
  const unit = active ? LUGHATI3_S2_UNITS[active.unitIdx] : null;
  ...
  const onAnswer = useCallback((val) => {
    setAnswered(true);
    setUserAns(val);
    const ok = val === '1' || val === question?.correct || val === question?.q?.correct;
    // for yesno / mc determine correctness
    let isOk = false;
    if (question.type === 'yesno') isOk = val === question.correct;
    else if (question.type === 'mc') isOk = val === question.correct;
    else isOk = val === '1'; // ordering / matching / classify / word_order pass '1' or '0'
    setCorrect(isOk);
    ...
    return (
      <div className="space-y-6" dir="rtl">
        <PageHeader
          ...
          breadcrumbs={[
            { label: t('home'), path: '/' },
            { label: 'لغتي — الصف الثالث ف٢', path: '/lughati-grade3-s2' },
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div className="flex items-center gap-2 pb-2 border-b border-slate-100">
                ...
                <span className="text-xs text-slate-400">من كتاب لغتي — الصف الثالث الابتدائي — الفصل الثاني</span>
              </div>
  ...
  return (
    <div className="space-y-8" dir="rtl">
      <PageHeader
        title="لغتي — الصف الثالث الابتدائي · الفصل الثاني"
        subtitle={`${toAr(LUGHATI3_S2_UNITS.length)} وحداتٍ · ${toAr(TOTAL_L3S2_QUESTIONS)} سؤالًا — مكارم الأخلاق · العمل · العلماء · البحار`}
        breadcrumbs={[{ label: t('home'), path: '/' }, { label: 'لغتي — الصف الثالث ف٢' }]}
      />
      ...
      <div className="max-w-4xl mx-auto px-4 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            { n: toAr(LUGHATI3_S2_UNITS.length), l: 'وحدة' },
            { n: toAr(LUGHATI3_S2_UNITS.reduce((s, u) => s + u.lessons.length, 0)), l: 'درسًا' },
            { n: toAr(TOTAL_L3S2_QUESTIONS), l: 'سؤالًا' },
          ].map((s) => (

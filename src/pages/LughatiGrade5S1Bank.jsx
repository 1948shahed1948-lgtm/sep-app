...
import { LUGHATI_G5_S1_UNITS, LUGHATI_G5_S1_TOTAL } from '@/lib/lughatiGrade5S1_Unit1';
...
function ExerciseRenderer({ question, onDone, answered, userAnswer, onAnswer }) {
  ...
  if (question.type === 'word_order') return <G5WordOrder question={question} onComplete={onDone} />;
  return null;
}

export default function LughatiGrade5S1Bank() {
  const { isRTL } = useLang();
  ...
  const next = () => {
    if (currentQ + 1 >= activeLesson.questions.length) { setDone(true); return; }
    setCurrentQ(i => i + 1); setAnswered(false);
    setUserAnswer(null); setShowResult(false); setLastAnswerCorrect(false);
  };
  ...
  if (done && activeLesson) {
    ...
    return (
      <div className="space-y-6">
        <PageHeader title="لغتي الجميلة — الصف الخامس ف١" backTo="/question-bank/grade_5/first" />
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
          ...
          <p className="text-5xl font-black text-emerald-600">{pct}%</p>
  ...
  if (activeLesson && q) {
    ...
    return (
      <div className="space-y-4">
        <PageHeader title="لغتي الجميلة — الصف الخامس ف١" subtitle={activeLesson.title_ar} backTo="/question-bank/grade_5/first" />
        ...
        <AnimatePresence mode="wait">
          <motion.div key={currentQ} initial={{ opacity: 0, x: isRTL ? -30 : 30 }}
            ...
            <ExerciseRenderer
              question={q} answered={answered} userAnswer={userAnswer}
              onAnswer={handleAnswer} onDone={handleInteractiveDone}
            />
            {showResult && (
              <WrongAnswerTeacher
            ...
            {(answered || (isInteractive && showResult)) && (
              <Button onClick={next} className="w-full mt-2 gap-2 bg-emerald-600 hover:bg-emerald-700">
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="لغتي الجميلة — الصف الخامس الابتدائي (ف١)"
        subtitle={`${LUGHATI_G5_S1_TOTAL} سؤالاً موزعةً على وحدة أخلاق وفضائل`}
      ...
      <div className="max-w-4xl mx-auto px-4 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            { n: toAr(LUGHATI_G5_S1_UNITS.length), l: 'وحدة' },
            { n: toAr(LUGHATI_G5_S1_UNITS.reduce((s, u) => s + u.lessons.length, 0)), l: 'درسًا' },
            { n: toAr(LUGHATI_G5_S1_TOTAL), l: 'سؤالًا' },
          ].map((s) => (
        ...
        {LUGHATI_G5_S1_UNITS.map((unit, ui) => (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {unit.lessons.map((lesson, li) => (
                <button
                  ...
                  className="group bg-white rounded-2xl border-2 border-slate-100 hover:border-emerald-300 hover:shadow-md transition-all p-4 text-right flex items-center gap-3"
                >

...
import { LUGHATI_G6_S2_UNITS as UNITS_P1, LUGHATI_G6_S2_TOTAL as TOTAL_P1 } from '@/lib/lughatiGrade6S2Bank';
import { LUGHATI_G6_S2_UNITS_PART2, LUGHATI_G6_S2_TOTAL_PART2 } from '@/lib/lughatiGrade6S2Bank_Part2';
import { LUGHATI_G6_S2_UNITS_PART3, LUGHATI_G6_S2_TOTAL_PART3 } from '@/lib/lughatiGrade6S2Bank_Part3';

const LUGHATI_G6_S2_UNITS = [...UNITS_P1, ...LUGHATI_G6_S2_UNITS_PART2, ...LUGHATI_G6_S2_UNITS_PART3];
const LUGHATI_G6_S2_TOTAL = TOTAL_P1 + LUGHATI_G6_S2_TOTAL_PART2 + LUGHATI_G6_S2_TOTAL_PART3;
          ...
          return (
            <button key={label} disabled={show} onClick={() => handleTF(val)}
              className={`flex-1 py-4 rounded-xl border-2 text-base font-bold transition-all
                ${!show ? 'bg-white border-slate-200 hover:border-violet-400 cursor-pointer' : ''}
                ${show && isCorrect ? 'bg-emerald-50 border-emerald-400 text-emerald-800' : ''}
                ${show && isPicked && !isCorrect ? 'bg-red-50 border-red-400 text-red-800' : ''}
            ...
            {q.pairs_ar.map((p, i) => (
              <button key={i} onClick={() => handleMatchPick('right', i)}
                className={`w-full text-right px-3 py-2 rounded-lg border-2 text-xs font-medium transition-all
                  ${matchSel?.side === 'right' && matchSel?.idx === i ? 'border-violet-500 bg-violet-50' : 'border-slate-200 bg-white hover:border-violet-300'}
          ...
          <div className="space-y-2">
            {q.pairs_ar.map((p, i) => (
              <button key={i} onClick={() => handleMatchPick('left', i)}
                className={`w-full text-right px-3 py-2 rounded-lg border-2 text-xs font-medium transition-all
                  ${matchSel?.side === 'left' && matchSel?.idx === i ? 'border-violet-500 bg-violet-50' : 'border-slate-200 bg-white hover:border-violet-300'}
      ...
      <div className="space-y-3">
        ...
        {orderItems.map((item, i) => (
          <div key={item} className="flex items-center gap-2 bg-white border-2 border-slate-200 rounded-xl px-3 py-2">
            <span className="w-6 h-6 rounded-full bg-violet-100 flex items-center justify-center text-xs font-bold text-violet-700 flex-shrink-0">{i + 1}</span>
        ...
        {!checked && <Button onClick={checkOrder} className="w-full bg-violet-600 hover:bg-violet-700">تحقق من الترتيب ✓</Button>}
      </div>
    );
  }
    ...
    <div ref={containerRef} className="space-y-4">
      <div className="flex items-center gap-3">
        <div className="flex-1 bg-slate-100 rounded-full h-2">
          <div className="bg-violet-500 h-2 rounded-full transition-all duration-500" style={{ width: `${(idx / questions.length) * 100}%` }} />
        </div>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={q.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
          ...
          <div className="flex items-start gap-2">
            <span className="text-xs bg-violet-100 text-violet-700 rounded-full px-2 py-0.5 font-bold flex-shrink-0 mt-0.5">{idx + 1}</span>
          ...
          {answered && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <Button onClick={next} className="w-full mt-3 bg-violet-600 hover:bg-violet-700 gap-2">
...
export default function LughatiGrade6S2() {
  const [activeUnit, setActiveUnit] = useState(null);
  const [activeLesson, setActiveLesson] = useState(null);

  if (activeUnit !== null && activeLesson !== null) {
    const unit = LUGHATI_G6_S2_UNITS[activeUnit];
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {unit.lessons.map((lesson, i) => (
              <motion.button key={lesson.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                ...
                className="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 text-right hover:shadow-md hover:border-violet-200 transition-all flex items-center gap-3">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-violet-50 to-purple-50" dir="rtl">
      <div className="relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #5b21b6 0%, #7c3aed 60%, #4f46e5 100%)' }}>
        ...
        <div className="relative max-w-3xl mx-auto px-4 py-10 sm:py-14 text-center text-white">
          ...
          <h1 className="text-2xl sm:text-3xl font-black mb-2">لغتي الجميلة - الصف السادس</h1>
          <p className="text-white/80 text-sm">الفصل الثاني | الوعي الصحي والاجتماعي · قواعد اللغة العربية</p>
          <div className="flex justify-center gap-4 mt-5 text-xs flex-wrap">
            <div className="bg-white/10 rounded-xl px-4 py-2">
              <p className="font-bold text-lg">{LUGHATI_G6_S2_UNITS.length}</p>
              <p className="text-white/70">وحدات</p>
            </div>
            <div className="bg-white/10 rounded-xl px-4 py-2">
              <p className="font-bold text-lg">{LUGHATI_G6_S2_UNITS.reduce((s, u) => s + u.lessons.length, 0)}</p>
              <p className="text-white/70">دروس</p>
            </div>
            <div className="bg-white/10 rounded-xl px-4 py-2">
              <p className="font-bold text-lg">{LUGHATI_G6_S2_TOTAL}</p>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-4">
        {LUGHATI_G6_S2_UNITS.map((unit, i) => (
          <motion.button key={unit.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            transition={{ delay: i * 0.1 }} onClick={() => setActiveUnit(i)}
            className="w-full bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-md hover:border-violet-200 transition-all text-right">
            <div className="flex items-center gap-4 p-5">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 mt-2">
                  <span className="text-xs bg-violet-100 text-violet-700 rounded-full px-2 py-0.5">{unit.lessons.length} دروس</span>
                  <span className="text-xs bg-purple-100 text-purple-700 rounded-full px-2 py-0.5">{unit.lessons.reduce((s, l) => s + l.count, 0)} سؤالاً</span>
                </div>
              </div>

...
import { MATH_G4_S1_UNITS, MATH_G4_S1_TOTAL } from '@/lib/mathGrade4S1';
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl bg-emerald-50 border-2 border-emerald-200 p-4 flex items-start gap-3">
        ...
        <div>
          <p className="text-xs font-bold text-emerald-700 mb-1">معلومة من الكتاب 📖</p>
          <p dir="rtl" className="text-sm text-emerald-900 leading-relaxed font-medium"><MathText text={q.question_ar} /></p>
        </div>
      </div>
      <Button onClick={onNext} className="w-full bg-emerald-600 hover:bg-emerald-700 gap-2">
          ...
          {q.pairs_ar.map((pair, i) => (
            <button key={i} onClick={() => handleMatchPick('right', i)}
              className={`w-full text-right px-3 py-2.5 rounded-lg border-2 text-xs font-medium transition-all
                ...
                ${connections[i] !== undefined ? 'border-emerald-400 bg-emerald-50' : ''}`}><MathText text={pair.right} /></button>
          ))}
        </div>
        <div className="space-y-2">
          {q.pairs_ar.map((pair, i) => (
            <button key={i} onClick={() => handleMatchPick('left', i)}
              className={`w-full text-right px-3 py-2.5 rounded-lg border-2 text-xs font-medium transition-all
                ...
                ${Object.values(connections).includes(i) ? 'border-emerald-400 bg-emerald-50' : ''}`}><MathText text={pair.left} /></button>
          ))}
        </div>
      </div>
    </div>
  ...
  if (q.type === 'ordering' && orderItems) {
    ...
    return (
      <div className="space-y-3">
        ...
        {!checked && <Button onClick={checkOrder} className="w-full bg-emerald-600 hover:bg-emerald-700">تحقق من الترتيب ✓</Button>}
      </div>
    );
  }
  return null;
    ...
    return (
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center py-10 px-4 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          ...
          <Button onClick={onBack} className="bg-emerald-600 hover:bg-emerald-700 gap-2"><ArrowRight className="w-4 h-4" />العودة</Button>
        </div>
      </motion.div>
    );
  }
        ...
        <motion.div key={q.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
          ...
          {isInfo ? <InfoCard q={q} onNext={next} /> : (
            <>
              ...
              {q.page && <span className="text-xs text-slate-400">📄 صفحة {q.page}</span>}
                    ...
                    <div className="space-y-2">
                      ...
                      {q.golden_rule_ar && (
                        <div className="rounded-xl bg-amber-50 border-2 border-amber-300 p-3 flex items-start gap-2">
                          ...
                          <p className="text-xs font-medium text-amber-900"><MathText text={q.golden_rule_ar} /></p>
                        </div>
                      )}
                    </div>
                  ) : (
...
export default function MathGrade4S1Bank() {
  const [activeUnit, setActiveUnit] = useState(null);
  const [activeLesson, setActiveLesson] = useState(null);

  if (activeUnit !== null && activeLesson !== null) {
    const unit = MATH_G4_S1_UNITS[activeUnit];
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {unit.lessons.map((lesson, i) => (
              <motion.button key={lesson.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} onClick={() => setActiveLesson(i)}
                ...
                <div className="flex-1 min-w-0"><p className="text-sm font-bold text-slate-800 leading-snug">{lesson.title_ar}</p><p className="text-xs text-slate-400 mt-0.5">{lesson.subtitle_ar}</p><p className="text-xs text-emerald-500 mt-0.5">{lesson.getQuestions().length} سؤالاً · ص {lesson.page}</p></div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50" dir="rtl">
      <div className="relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #1e3a6e 0%, #2563eb 60%, #1e40af 100%)' }}>
        ...
        <div className="relative max-w-3xl mx-auto px-4 py-10 sm:py-14 text-center text-white">
          <Link to="/question-bank/grades" className="inline-flex items-center gap-1.5 text-xs text-white/60 hover:text-white/90 mb-4 transition-colors">
            <Home className="w-3.5 h-3.5" />بنوك الأسئلة
          </Link>
          <div className="text-5xl mb-4">🔢</div>
          <h1 className="text-2xl sm:text-3xl font-black mb-2">رياضيات الصف الرابع</h1>
          <p className="text-white/80 text-sm">الفصل الدراسي الأول | القيمة المنزلية والجمع والطرح</p>
          <div className="flex justify-center gap-4 mt-5 text-xs flex-wrap">
            <div className="bg-white/10 rounded-xl px-4 py-2"><p className="font-bold text-lg">{MATH_G4_S1_UNITS.length}</p><p className="text-white/70">فصول</p></div>
            <div className="bg-white/10 rounded-xl px-4 py-2"><p className="font-bold text-lg">{MATH_G4_S1_UNITS.reduce((s, u) => s + u.lessons.length, 0)}</p><p className="text-white/70">دروس</p></div>
            <div className="bg-white/10 rounded-xl px-4 py-2"><p className="font-bold text-lg">{MATH_G4_S1_TOTAL}</p><p className="text-white/70">سؤالاً</p></div>
          </div>
        </div>
      </div>
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-4">
        {MATH_G4_S1_UNITS.map((unit, i) => (
            ...
            <div className="flex items-center gap-4 p-5">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 mt-2">
                  ...
                  <span className="text-xs bg-emerald-100 text-emerald-700 rounded-full px-2 py-0.5">{unit.lessons.reduce((s, l) => s + l.getQuestions().length, 0)} سؤالاً</span>
                </div>
              </div>

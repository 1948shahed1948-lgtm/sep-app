...
import { MATH_G5_S2_CH7_UNITS, MATH_G5_S2_CH7_TOTAL } from '@/lib/mathGrade5S2_Ch7';
import WrongAnswerTeacher from '@/components/aptitude/WrongAnswerTeacher';

function InfoCard({ q, onNext }) {
  return (
    <div className="space-y-4">
      <div className="rounded-xl bg-indigo-50 border-2 border-indigo-200 p-4 flex items-start gap-3">
        <div className="flex-shrink-0 w-8 h-8 rounded-full bg-indigo-500 flex items-center justify-center">
          <Lightbulb className="w-4 h-4 text-white" />
        </div>
        <div>
          <p className="text-xs font-bold text-indigo-700 mb-1">معلومة من الكتاب 📖</p>
          <p dir="rtl" className="text-sm text-indigo-900 leading-relaxed font-medium">{q.question_ar}</p>
        </div>
      </div>
...
function QuestionRenderer({ q, onAnswer, isAnswered }) {
  ...
  const checkOrder = () => {
    ...
    setSelected('checked'); onAnswer(orderItems.join('|') === correct.join('|'));
  };
          ...
          return (
            <button key={label} disabled={show} onClick={() => handleTF(val)}
              className={`flex-1 py-4 rounded-xl border-2 text-base font-bold transition-all
                ${!show ? 'bg-white border-slate-200 hover:border-indigo-400 cursor-pointer' : ''}
                ${show && isCorrect ? 'bg-emerald-50 border-emerald-400 text-emerald-800' : ''}
                ${show && isPicked && !isCorrect ? 'bg-red-50 border-red-400 text-red-800' : ''}
...
function LessonRunner({ lesson, onBack }) {
  ...
  const handleAnswer = (correct) => {
    ...
    setTimeout(() => { if (containerRef.current) window.scrollTo({ top: containerRef.current.getBoundingClientRect().top + window.scrollY - 80, behavior: 'smooth' }); }, 300);
  };
...
export default function MathGrade5S2Ch7() {
  const [activeUnit, setActiveUnit] = useState(null);
  const [activeLesson, setActiveLesson] = useState(null);

  if (activeUnit !== null && activeLesson !== null) {
    const unit = MATH_G5_S2_CH7_UNITS[activeUnit];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-blue-50" dir="rtl">
      <div className="relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #3730a3 0%, #4f46e5 60%, #6d28d9 100%)' }}>
        ...
        <div className="relative max-w-3xl mx-auto px-4 py-10 sm:py-14 text-center text-white">
          ...
          <p className="text-white/80 text-sm">الفصل السابع: الإحصاء والاحتمال</p>
          <div className="flex justify-center gap-4 mt-5 text-xs flex-wrap">
            <div className="bg-white/10 rounded-xl px-4 py-2"><p className="font-bold text-lg">{MATH_G5_S2_CH7_UNITS.length}</p><p className="text-white/70">دروس</p></div>
            <div className="bg-white/10 rounded-xl px-4 py-2"><p className="font-bold text-lg">{MATH_G5_S2_CH7_UNITS.reduce((s, u) => s + u.lessons.length, 0)}</p><p className="text-white/70">وحدات</p></div>
            <div className="bg-white/10 rounded-xl px-4 py-2"><p className="font-bold text-lg">{MATH_G5_S2_CH7_TOTAL}</p><p className="text-white/70">سؤالاً</p></div>
          </div>
        </div>
      </div>
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-4">
        {MATH_G5_S2_CH7_UNITS.map((unit, i) => (
            ...
            <div className="flex items-center gap-4 p-5">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 mt-2">
                  <span className="text-xs bg-indigo-100 text-indigo-700 rounded-full px-2 py-0.5">{unit.lessons.length} وحدة</span>

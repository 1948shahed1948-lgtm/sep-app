...
import { MATH_G5_S2_CH8_UNITS, MATH_G5_S2_CH8_TOTAL } from '@/lib/mathGrade5S2_Ch8';
        ...
        <motion.div key={q.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
          ...
          {isInfo ? <InfoCard q={q} onNext={next} /> : (
            <>
              ...
              {q.image_url && <div className="rounded-xl overflow-hidden border border-slate-100 bg-slate-50"><img src={q.image_url} alt="" className="w-full max-h-48 object-contain" /></div>}
...
export default function MathGrade5S2Ch8() {
  const [activeUnit, setActiveUnit] = useState(null);
  const [activeLesson, setActiveLesson] = useState(null);

  if (activeUnit !== null && activeLesson !== null) {
    const unit = MATH_G5_S2_CH8_UNITS[activeUnit];
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-teal-50" dir="rtl">
      <div className="relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #065f46 0%, #059669 60%, #047857 100%)' }}>
        ...
        <div className="relative max-w-3xl mx-auto px-4 py-10 sm:py-14 text-center text-white">
          ...
          <p className="text-white/80 text-sm">الفصل الثاني | الفصل الثامن: القواسم والمضاعفات</p>
          <div className="flex justify-center gap-4 mt-5 text-xs flex-wrap">
            <div className="bg-white/10 rounded-xl px-4 py-2"><p className="font-bold text-lg">{MATH_G5_S2_CH8_UNITS.length}</p><p className="text-white/70">دروس</p></div>
            <div className="bg-white/10 rounded-xl px-4 py-2"><p className="font-bold text-lg">{MATH_G5_S2_CH8_UNITS.reduce((s, u) => s + u.lessons.length, 0)}</p><p className="text-white/70">وحدات</p></div>
            <div className="bg-white/10 rounded-xl px-4 py-2"><p className="font-bold text-lg">{MATH_G5_S2_CH8_TOTAL}</p><p className="text-white/70">سؤالاً</p></div>
          </div>
        </div>
      </div>
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-4">
        {MATH_G5_S2_CH8_UNITS.map((unit, i) => (
            ...
            <div className="flex items-center gap-4 p-5">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 mt-2">
                  <span className="text-xs bg-emerald-100 text-emerald-700 rounded-full px-2 py-0.5">{unit.lessons.length} وحدات</span>

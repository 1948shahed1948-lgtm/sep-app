...
import { MATH_G6_CH6_UNITS, MATH_G6_CH6_TOTAL } from '@/lib/mathGrade6Ch6Bank';
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl bg-blue-50 border-2 border-blue-200 p-4 flex items-start gap-3">
        ...
        <div>
          <p className="text-xs font-bold text-blue-700 mb-1">مفهوم أساسي من الكتاب 📖</p>
          <p dir="rtl" className="text-sm text-blue-900 leading-relaxed font-medium whitespace-pre-line"><MathText text={q.question_ar} /></p>
        </div>
      </div>
      <Button onClick={onNext} className="w-full bg-blue-600 hover:bg-blue-700 gap-2">فهمت — السؤال التالي <ArrowRight className="w-4 h-4" /></Button>
    </div>
  );
}
...
function QuestionRenderer({ q, onAnswer, isAnswered }) {
  ...
  const handleMatchPick = (side, idx) => {
    ...
    if (Object.keys(nc).length === q.pairs_ar.length) onAnswer(Object.entries(nc).every(([r, l]) => parseInt(r) === l));
  };
      ...
      {q.options_ar.map((opt, i) => {
        ...
        return (
          <button key={i} disabled={show} onClick={() => handleMC(i)}
            className={`w-full text-right px-4 py-3 rounded-xl border-2 text-sm font-medium transition-all ${!show ? COLORS[i % 4] + ' cursor-pointer' : ''} ${show && isCorrect ? 'bg-emerald-50 border-emerald-400 text-emerald-800' : ''} ${show && isPicked && !isCorrect ? 'bg-red-50 border-red-400 text-red-800' : ''} ${show && !isPicked && !isCorrect ? 'bg-white border-slate-200 text-slate-400 opacity-70' : ''}`}>
            <span className="flex items-center gap-2">{show && isCorrect && <CheckCircle2 className="w-4 h-4 text-emerald-600 flex-shrink-0" />}{show && isPicked && !isCorrect && <XCircle className="w-4 h-4 text-red-500 flex-shrink-0" />}<MathText text={opt} /></span>
          </button>
        );
      })}
    </div>
  ...
  if (q.type === 'classify' && dragItems !== null) {
    ...
    return (
      <div className="space-y-4">
        {!done && (<div className="rounded-xl border-2 border-dashed border-amber-300 bg-amber-50 p-4 text-center"><p className="text-xs text-amber-700 font-bold mb-2">صنِّف هذا العنصر:</p><span className="text-sm font-bold text-slate-800 bg-white px-4 py-2 rounded-lg shadow-sm border"><MathText text={dragItems[0].text} /></span><div className="flex flex-wrap gap-2 justify-center mt-3">{q.categories_ar.map((cat, i) => (<button key={i} onClick={() => handleClassifyPick(i)} className={`px-4 py-2 rounded-lg text-white text-sm font-bold transition-colors ${catColors[i % 3]}`}>{cat}</button>))}</div></div>)}
        <div className={`grid gap-3 ${q.categories_ar.length === 3 ? 'grid-cols-3' : 'grid-cols-2'}`}>{bins && bins.map((bin, i) => (<div key={i} className={`rounded-xl border-2 p-3 min-h-[60px] ${binColors[i % 3]}`}><p className={`text-xs font-bold mb-2 ${textColors[i % 3]}`}>{q.categories_ar[i]}</p><div className="flex flex-wrap gap-1">{bin.map((it, j) => <span key={j} className="text-xs bg-white border rounded-full px-2 py-0.5 text-slate-700"><MathText text={it.text} /></span>)}</div></div>))}</div>
      </div>
    );
  }

  if (q.type === 'matching') return (
    <div className="space-y-3"><p className="text-xs text-slate-500 text-center">اضغط من العمود الأيمن ثم من الأيسر:</p><div className="grid grid-cols-2 gap-3"><div className="space-y-2">{q.pairs_ar.map((pair, i) => (<button key={i} onClick={() => handleMatchPick('right', i)} className={`w-full text-right px-3 py-2.5 rounded-lg border-2 text-xs font-medium transition-all ${matchSel?.side === 'right' && matchSel?.idx === i ? 'border-blue-500 bg-blue-50' : 'border-slate-200 bg-white hover:border-blue-300'} ${connections[i] !== undefined ? 'border-emerald-400 bg-emerald-50' : ''}`}><MathText text={pair.right} /></button>))}</div><div className="space-y-2">{q.pairs_ar.map((pair, i) => (<button key={i} onClick={() => handleMatchPick('left', i)} className={`w-full text-right px-3 py-2.5 rounded-lg border-2 text-xs font-medium transition-all ${matchSel?.side === 'left' && matchSel?.idx === i ? 'border-blue-500 bg-blue-50' : 'border-slate-200 bg-white hover:border-blue-300'} ${Object.values(connections).includes(i) ? 'border-emerald-400 bg-emerald-50' : ''}`}><MathText text={pair.left} /></button>))}</div></div></div>
  );

  if (q.type === 'ordering' && orderItems) {
    const checked = selected === 'checked';
    return (
      <div className="space-y-3"><p className="text-xs text-slate-500">استخدم الأسهم لإعادة الترتيب:</p>{orderItems.map((item, i) => (<div key={item} className="flex items-center gap-2 bg-white border-2 border-slate-200 rounded-xl px-3 py-2.5"><span className="w-6 h-6 rounded-full bg-blue-100 flex items-center justify-center text-xs font-bold text-blue-700 flex-shrink-0">{i + 1}</span><span className="flex-1 text-xs sm:text-sm font-medium text-slate-800 text-right"><MathText text={item} /></span><div className="flex flex-col gap-0.5"><button disabled={i === 0 || checked} onClick={() => moveItem(i, i - 1)} className="text-slate-400 hover:text-slate-700 disabled:opacity-30 text-xs">▲</button><button disabled={i === orderItems.length - 1 || checked} onClick={() => moveItem(i, i + 1)} className="text-slate-400 hover:text-slate-700 disabled:opacity-30 text-xs">▼</button></div></div>))}{!checked && <Button onClick={checkOrder} className="w-full bg-blue-600 hover:bg-blue-700">تحقق من الترتيب ✓</Button>}</div>
    );
  }
  return null;
}
    ...
    return (
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center py-10 px-4 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          ...
          <Button onClick={onBack} className="bg-blue-600 hover:bg-blue-700 gap-2"><ArrowRight className="w-4 h-4" />العودة</Button>
        </div>
      </motion.div>
    );
  }
  ...
  return (
    <div ref={containerRef} className="space-y-4">
      <div className="flex items-center gap-3">
        <div className="flex-1 bg-slate-100 rounded-full h-2"><div className="bg-blue-500 h-2 rounded-full transition-all duration-500" style={{ width: `${(idx / questions.length) * 100}%` }} /></div>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={q.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 space-y-4">
          {isInfo ? <InfoCard q={q} onNext={next} /> : (
            <>
              <div className="flex items-start gap-2"><span className="text-xs bg-blue-100 text-blue-700 rounded-full px-2 py-0.5 font-bold flex-shrink-0 mt-0.5">{idx + 1}</span><p className="text-base font-bold text-slate-800 leading-relaxed"><MathText text={q.question_ar} /></p></div>
                ...
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
                  {lastCorrect ? (
                    <div className="space-y-2">
                      ...
                      {q.golden_rule && <div className="rounded-xl bg-amber-50 border-2 border-amber-300 p-3 flex items-start gap-2"><Crown className="w-4 h-4 text-amber-600 flex-shrink-0 mt-0.5" /><p className="text-xs font-medium text-amber-900"><MathText text={q.golden_rule} /></p></div>}
                    </div>
                  ) : <WrongAnswerTeacher explanation={q.explanation_ar} goldenRule={q.golden_rule || q.explanation_ar} lang="ar" isRTL={true} />}
                  <Button onClick={next} className="w-full mt-3 bg-blue-600 hover:bg-blue-700 gap-2">{idx + 1 >= questions.length ? <><Trophy className="w-4 h-4" />إنهاء الدرس</> : <>السؤال التالي<ArrowRight className="w-4 h-4" /></>}</Button>
                </motion.div>
              )}
            </>
          )}
...
export default function MathGrade6Ch6() {
  const [activeUnit, setActiveUnit] = useState(null);
  const [activeLesson, setActiveLesson] = useState(null);

  if (activeUnit !== null && activeLesson !== null) {
    const unit = MATH_G6_CH6_UNITS[activeUnit];
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50 p-4 sm:p-6" dir="rtl">
        <div className="max-w-2xl mx-auto space-y-4">
          <div className={`rounded-2xl p-5 text-white bg-gradient-to-r ${unit.color}`}>
            <div className="flex items-center gap-3">
              <button onClick={() => setActiveLesson(null)} className="w-9 h-9 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center"><ChevronRight className="w-5 h-5 text-white" /></button>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {unit.lessons.map((lesson, i) => (
              <motion.button key={lesson.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} onClick={() => setActiveLesson(i)}
                className="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 text-right hover:shadow-md hover:border-blue-200 transition-all flex items-center gap-3">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50" dir="rtl">
      <div className="relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #1e3a8a 0%, #2563eb 60%, #7c3aed 100%)' }}>
        ...
        <div className="relative max-w-3xl mx-auto px-4 py-10 sm:py-14 text-center text-white">
          ...
          <p className="text-white/80 text-sm">الفصل السادس: العمليات على الكسور الاعتيادية</p>
          <div className="flex justify-center gap-4 mt-5 text-xs flex-wrap">
            <div className="bg-white/10 rounded-xl px-4 py-2"><p className="font-bold text-lg">{MATH_G6_CH6_UNITS.length}</p><p className="text-white/70">دروس</p></div>
            <div className="bg-white/10 rounded-xl px-4 py-2"><p className="font-bold text-lg">{MATH_G6_CH6_UNITS.reduce((s, u) => s + u.lessons.length, 0)}</p><p className="text-white/70">وحدات</p></div>
            <div className="bg-white/10 rounded-xl px-4 py-2"><p className="font-bold text-lg">{MATH_G6_CH6_TOTAL}</p><p className="text-white/70">سؤالاً</p></div>
          </div>
        </div>
      </div>
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-4">
        {MATH_G6_CH6_UNITS.map((unit, i) => (
          <motion.button key={unit.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} onClick={() => setActiveUnit(i)}
            ...
            <div className="flex items-center gap-4 p-5">
              <div className={`w-14 h-14 rounded-xl flex-shrink-0 bg-gradient-to-br ${unit.color} flex items-center justify-center text-2xl font-black text-white`}>{unit.icon}</div>
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 mt-2">
                  <span className="text-xs bg-blue-100 text-blue-700 rounded-full px-2 py-0.5">{unit.lessons.length} {unit.lessons.length === 1 ? 'درس' : 'دروس'}</span>
                  <span className="text-xs bg-indigo-100 text-indigo-700 rounded-full px-2 py-0.5">{unit.lessons.reduce((s, l) => s + l.count, 0)} سؤالاً</span>
                </div>
              </div>

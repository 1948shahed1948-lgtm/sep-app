...
import { MATH_G6_CH8_UNITS, MATH_G6_CH8_TOTAL } from '@/lib/mathGrade6Ch8Bank';
import WrongAnswerTeacher from '@/components/aptitude/WrongAnswerTeacher';

function InfoCard({ q, onNext }) {
  return (
    <div className="space-y-4">
      <div className="rounded-xl bg-violet-50 border-2 border-violet-200 p-4 flex items-start gap-3">
        <div className="flex-shrink-0 w-8 h-8 rounded-full bg-violet-500 flex items-center justify-center"><Lightbulb className="w-4 h-4 text-white" /></div>
        <div>
          <p className="text-xs font-bold text-violet-700 mb-1">مفهوم أساسي 📖</p>
          <p dir="rtl" className="text-sm text-violet-900 leading-relaxed font-medium whitespace-pre-line">{q.question_ar}</p>
        </div>
      </div>
      <Button onClick={onNext} className="w-full bg-violet-600 hover:bg-violet-700 gap-2">فهمت — السؤال التالي <ArrowRight className="w-4 h-4" /></Button>
    </div>
  );
}
...
function QuestionRenderer({ q, onAnswer, isAnswered }) {
  ...
  if (q.type === 'ordering' && orderItems) {
    ...
    return (<div className="space-y-3"><p className="text-xs text-slate-500">استخدم الأسهم لإعادة الترتيب:</p>{orderItems.map((item, i) => (<div key={item} className="flex items-center gap-2 bg-white border-2 border-slate-200 rounded-xl px-3 py-2.5"><span className="w-6 h-6 rounded-full bg-violet-100 flex items-center justify-center text-xs font-bold text-violet-700 flex-shrink-0">{i + 1}</span><span className="flex-1 text-xs sm:text-sm font-medium text-slate-800 text-right">{item}</span><div className="flex flex-col gap-0.5"><button disabled={i === 0 || checked} onClick={() => moveItem(i, i - 1)} className="text-slate-400 hover:text-slate-700 disabled:opacity-30 text-xs">▲</button><button disabled={i === orderItems.length - 1 || checked} onClick={() => moveItem(i, i + 1)} className="text-slate-400 hover:text-slate-700 disabled:opacity-30 text-xs">▼</button></div></div>))}{!checked && <Button onClick={checkOrder} className="w-full bg-violet-600 hover:bg-violet-700">تحقق من الترتيب ✓</Button>}</div>);
  }
  return null;
}
    ...
    return (
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center py-10 px-4 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          ...
          <Button onClick={onBack} className="bg-violet-600 hover:bg-violet-700 gap-2"><ArrowRight className="w-4 h-4" />العودة</Button>
        </div>
      </motion.div>
    );
  }
  ...
  return (
    <div ref={containerRef} className="space-y-4">
      <div className="flex items-center gap-3">
        <div className="flex-1 bg-slate-100 rounded-full h-2"><div className="bg-violet-500 h-2 rounded-full transition-all duration-500" style={{ width: `${(idx / questions.length) * 100}%` }} /></div>
        <span className="text-xs font-medium text-slate-500">{idx + 1} / {questions.length}</span>
        <span className="text-xs font-bold text-violet-600">✅ {score}</span>
      </div>
      <AnimatePresence mode="wait">
        <motion.div key={q.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 space-y-4">
          {q.type === 'info' ? <InfoCard q={q} onNext={next} /> : (
            <>
              <div className="flex items-start gap-2"><span className="text-xs bg-violet-100 text-violet-700 rounded-full px-2 py-0.5 font-bold flex-shrink-0 mt-0.5">{idx + 1}</span><p className="text-base font-bold text-slate-800 leading-relaxed">{q.question_ar}</p></div>
              ...
              {answered && (
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
                  ...
                  <Button onClick={next} className="w-full mt-3 bg-violet-600 hover:bg-violet-700 gap-2">{idx + 1 >= questions.length ? <><Trophy className="w-4 h-4" />إنهاء الدرس</> : <>السؤال التالي<ArrowRight className="w-4 h-4" /></>}</Button>
                </motion.div>
              )}
            </>
          )}
...
export default function MathGrade6Ch8() {
  const [activeUnit, setActiveUnit] = useState(null);
  const [activeLesson, setActiveLesson] = useState(null);

  if (activeUnit !== null && activeLesson !== null) {
    const unit = MATH_G6_CH8_UNITS[activeUnit];
    const lesson = unit.lessons[activeLesson];
    return (
      <div className="min-h-screen bg-gradient-to-br from-violet-50 to-purple-50 p-4 sm:p-6" dir="rtl">
        ...
        <div className="max-w-2xl mx-auto space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {unit.lessons.map((lesson, i) => (
              <motion.button key={lesson.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} onClick={() => setActiveLesson(i)} className="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 text-right hover:shadow-md hover:border-violet-200 transition-all flex items-center gap-3">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-violet-50 to-purple-50" dir="rtl">
      <div className="relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #4c1d95 0%, #7c3aed 60%, #a855f7 100%)' }}>
        ...
        <div className="relative max-w-3xl mx-auto px-4 py-10 sm:py-14 text-center text-white">
          ...
          <p className="text-white/80 text-sm">الفصل الثامن: النسبة المئوية والاحتمالات</p>
          <div className="flex justify-center gap-4 mt-5 text-xs flex-wrap">
            <div className="bg-white/10 rounded-xl px-4 py-2"><p className="font-bold text-lg">{MATH_G6_CH8_UNITS.length}</p><p className="text-white/70">دروس</p></div>
            <div className="bg-white/10 rounded-xl px-4 py-2"><p className="font-bold text-lg">{MATH_G6_CH8_UNITS.reduce((s, u) => s + u.lessons.length, 0)}</p><p className="text-white/70">وحدات</p></div>
            <div className="bg-white/10 rounded-xl px-4 py-2"><p className="font-bold text-lg">{MATH_G6_CH8_TOTAL}</p><p className="text-white/70">سؤالاً</p></div>
          </div>
        </div>
      </div>
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-4">
        {MATH_G6_CH8_UNITS.map((unit, i) => (
          <motion.button key={unit.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} onClick={() => setActiveUnit(i)} className="w-full bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-md hover:border-violet-200 transition-all text-right">
            <div className="flex items-center gap-4 p-5">
              <div className={`w-14 h-14 rounded-xl flex-shrink-0 bg-gradient-to-br ${unit.color} flex items-center justify-center text-2xl`}>{unit.icon}</div>
              <div className="flex-1"><h2 className="text-base font-bold text-slate-800">{unit.title_ar}</h2><p className="text-xs text-slate-500 mt-0.5">{unit.subtitle_ar}</p><div className="flex items-center gap-3 mt-2"><span className="text-xs bg-violet-100 text-violet-700 rounded-full px-2 py-0.5">{unit.lessons.length} درس</span><span className="text-xs bg-purple-100 text-purple-700 rounded-full px-2 py-0.5">{unit.lessons.reduce((s, l) => s + l.count, 0)} سؤالاً</span></div></div>
              <Star className="w-5 h-5 text-violet-400 flex-shrink-0" />
            </div>
          </motion.button>
        ))}
      </div>

...
import { MATH_G6_CH9_UNITS, MATH_G6_CH9_TOTAL } from '@/lib/mathGrade6Ch9Bank';
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl bg-blue-50 border-2 border-blue-200 p-4 flex items-start gap-3">
        <div className="flex-shrink-0 w-8 h-8 rounded-full bg-blue-700 flex items-center justify-center"><Lightbulb className="w-4 h-4 text-white" /></div>
        <div>
          <p className="text-xs font-bold text-blue-800 mb-1">مفهوم هندسي 📐</p>
          <p dir="rtl" className="text-sm text-blue-900 leading-relaxed font-medium whitespace-pre-line">{q.question_ar}</p>
        </div>
      </div>
      <Button onClick={onNext} className="w-full bg-blue-700 hover:bg-blue-800 gap-2">فهمت — السؤال التالي <ArrowRight className="w-4 h-4" /></Button>
    </div>
  );
}
  ...
  return (
    <div ref={containerRef} className="space-y-4">
      <div className="flex items-center gap-3">
        <div className="flex-1 bg-slate-100 rounded-full h-2"><div className="bg-blue-700 h-2 rounded-full transition-all duration-500" style={{ width: `${(idx / questions.length) * 100}%` }} /></div>
            ...
            <>
              ...
              {answered && (
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
                  ...
                  <Button onClick={next} className="w-full mt-3 bg-blue-700 hover:bg-blue-800 gap-2">{idx + 1 >= questions.length ? <><Trophy className="w-4 h-4" />إنهاء الدرس</> : <>السؤال التالي<ArrowRight className="w-4 h-4" /></>}</Button>
                </motion.div>
              )}
            </>
          )}
...
export default function MathGrade6Ch9() {
  const [activeUnit, setActiveUnit] = useState(null);
  const [activeLesson, setActiveLesson] = useState(null);

  if (activeUnit !== null && activeLesson !== null) {
    const unit = MATH_G6_CH9_UNITS[activeUnit];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50" dir="rtl">
      <div className="relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #1e3a8a 0%, #1d4ed8 60%, #7c3aed 100%)' }}>
        ...
        <div className="relative max-w-3xl mx-auto px-4 py-10 sm:py-14 text-center text-white">
          ...
          <p className="text-white/80 text-sm">الفصل التاسع: الهندسة — الزوايا والمضلعات</p>
          <div className="flex justify-center gap-4 mt-5 text-xs flex-wrap">
            <div className="bg-white/10 rounded-xl px-4 py-2"><p className="font-bold text-lg">{MATH_G6_CH9_UNITS.length}</p><p className="text-white/70">دروس</p></div>
            <div className="bg-white/10 rounded-xl px-4 py-2"><p className="font-bold text-lg">{MATH_G6_CH9_UNITS.reduce((s, u) => s + u.lessons.length, 0)}</p><p className="text-white/70">وحدات</p></div>
            <div className="bg-white/10 rounded-xl px-4 py-2"><p className="font-bold text-lg">{MATH_G6_CH9_TOTAL}</p><p className="text-white/70">سؤالاً</p></div>
          </div>
        </div>
      </div>
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-4">
        {MATH_G6_CH9_UNITS.map((unit, i) => (
          <motion.button key={unit.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} onClick={() => setActiveUnit(i)} className="w-full bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-md hover:border-blue-200 transition-all text-right">
            <div className="flex items-center gap-4 p-5">
              ...
              <div className="flex-1"><h2 className="text-base font-bold text-slate-800">{unit.title_ar}</h2><p className="text-xs text-slate-500 mt-0.5">{unit.subtitle_ar}</p><div className="flex items-center gap-3 mt-2"><span className="text-xs bg-blue-100 text-blue-700 rounded-full px-2 py-0.5">{unit.lessons.length} درس</span><span className="text-xs bg-indigo-100 text-indigo-700 rounded-full px-2 py-0.5">{unit.lessons.reduce((s, l) => s + l.count, 0)} سؤالاً</span></div></div>

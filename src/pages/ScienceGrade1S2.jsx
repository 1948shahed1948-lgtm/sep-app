...
import { ArrowLeft, ArrowRight, CheckCircle2, Crown, BookOpen } from 'lucide-react';
...
import { SCIENCE_S2_UNITS, getExercisesForScienceS2Lesson, TOTAL_LESSONS_SCI_S2, TOTAL_QUESTIONS_SCI_S2, getAdaptiveLadderS2, WORD_EMOJI_SCI_S2 } from '@/lib/scienceGrade1S2';
...
const BANNER = 'https://images.unsplash.com/photo-1532094349884-543bc11b234d?w=1200&q=80';
...
export default function ScienceGrade1S2() {
  ...
  const exercises = useMemo(
    () => (active ? withQuestionImages(getExercisesForScienceS2Lesson(active.unitIdx, active.lessonIdx), 'science') : []),
    [active]
  );
  ...
  const nextLesson = () => {
    ...
    const unit = SCIENCE_S2_UNITS[unitIdx];
    if (lessonIdx < unit.lessons.length - 1) {
      openLesson(unitIdx, lessonIdx + 1);
    } else if (unitIdx < SCIENCE_S2_UNITS.length - 1) {
  ...
  if (active && !done && exercise) {
    const unit = SCIENCE_S2_UNITS[active.unitIdx];
    ...
    return (
      <div className="space-y-6" dir="rtl">
        <PageHeader
          ...
          breadcrumbs={[{ label: t('home'), path: '/' }, { label: 'العلوم — الصف الأول ف٢', path: '/science-grade1-s2' }, { label: lesson.title_ar }]}
        />
        <div className="max-w-2xl mx-auto space-y-5">
          ...
          <div className="flex items-center gap-3">
            <div className={`w-12 h-12 rounded-xl ${th.tile} flex items-center justify-center flex-shrink-0 text-2xl`}>
              {WORD_EMOJI_SCI_S2[lesson.words?.[0]] || '🔬'}
            </div>
  ...
  const renderLessonTile = (l, li, th, onClick) => (
    <button onClick={onClick} className={`group flex flex-col items-center gap-2 p-3 rounded-2xl border-2 ${th.tileBorder} ${th.hover} hover:shadow-md transition-all hover:-translate-y-0.5`}>
      <div className={`w-16 h-16 rounded-2xl ${th.tile} flex items-center justify-center group-hover:scale-110 transition-transform text-3xl`}>
        {WORD_EMOJI_SCI_S2[l.words?.[0]] || '🔬'}
      </div>
      <div className="text-center">
        <div className={`text-xs font-bold ${th.num}`}>الدرس {toAr(li + 1)}</div>
        <div className={`text-xs font-medium ${th.tileText} mt-0.5 leading-tight`}>{l.title_ar}</div>
        <div className="flex items-center gap-1 justify-center flex-wrap mt-1">
          {l.words?.slice(0, 3).map((w, wi) => <span key={wi} className="text-lg leading-none" title={w}>{WORD_EMOJI_SCI_S2[w] || '🔬'}</span>)}
        </div>
      </div>
    </button>
  );
  ...
  return (
    <InteractiveBankBrowser
      units={SCIENCE_S2_UNITS}
      unitTheme={UNIT_THEME}
      selectedUnit={selectedUnit}
      onSelectUnit={setSelectedUnit}
      onPickLesson={openLesson}
      banner={BANNER}
      title="العلوم — الصف الأول الابتدائي · ف٢"
      subtitle={`الطقس · المادة · الطاقة (${toAr(SCIENCE_S2_UNITS.length)} وحدات · ${toAr(TOTAL_LESSONS_SCI_S2)} درسًا · ${toAr(TOTAL_QUESTIONS_SCI_S2)} سؤالًا)`}
      bankLabel="العلوم — الصف الأول ف٢"
      homeLabel={t('home')}
      backTo="/question-bank/grade_1/second"
      toAr={toAr}
      stats={[{ n: toAr(SCIENCE_S2_UNITS.length), l: 'وحدات' }, { n: toAr(TOTAL_LESSONS_SCI_S2), l: 'درسًا' }, { n: toAr(TOTAL_QUESTIONS_SCI_S2), l: 'سؤال' }]}

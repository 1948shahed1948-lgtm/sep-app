...
import {
  ArrowLeft, ArrowRight, CheckCircle2, Crown, BookOpen, ArrowRight as BackIcon,
  Globe, FlaskRound, Atom, Zap, Leaf, Rocket, Moon, Sun, Wind, Droplet, Flame,
} from 'lucide-react';
...
import {
  SCIENCE_G2_S2_UNITS, TOTAL_UNITS_SCI_G2S2, TOTAL_LESSONS_SCI_G2S2, TOTAL_QUESTIONS_SCI_G2S2,
} from '@/lib/scienceGrade2S2';
...
const UNIT_THEME = [
  ...
  { bar: 'bg-sky-500', iconBg: 'bg-sky-100', iconText: 'text-sky-700', tile: 'bg-sky-50', tileText: 'text-sky-700', tileBorder: 'border-sky-200', hover: 'hover:border-sky-400', num: 'text-sky-600' },
  { bar: 'bg-emerald-500', iconBg: 'bg-emerald-100', iconText: 'text-emerald-700', tile: 'bg-emerald-50', tileText: 'text-emerald-700', tileBorder: 'border-emerald-200', hover: 'hover:border-emerald-400', num: 'text-emerald-600' },
  { bar: 'bg-amber-500', iconBg: 'bg-amber-100', iconText: 'text-amber-700', tile: 'bg-amber-50', tileText: 'text-amber-700', tileBorder: 'border-amber-200', hover: 'hover:border-amber-400', num: 'text-amber-600' },
];

const LESSON_ICONS = { Globe, FlaskRound, Atom, Zap, Leaf, Rocket, Moon, Sun, Wind, Droplet, Flame, BookOpen };

const toAr = (n) => String(n).replace(/[0-9]/g, (d) => '٠١٢٣٤٥٦٧٨٩'[d]);

// محوّل الحقول: يربط أسماء مكتبة العلوم بالمكوّنات القياسية
...
function ExerciseRenderer({ exercise, isAnswered, onAnswer }) {
  ...
  if (ex.type === 'word_sentence_ordering') return <>{ImageBlock}<WordSentenceOrdering exercise={ex} lang="ar" isAnswered={isAnswered} onAnswer={onAnswer} /></>;
...
export default function ScienceGrade2S2() {
  ...
  const exercises = useMemo(() => {
    if (!active) return [];
    return withQuestionImages(SCIENCE_G2_S2_UNITS[active.unitIdx].lessons[active.lessonIdx].questions, 'science');
  }, [active]);
  ...
  const nextLesson = () => {
    ...
    const u = SCIENCE_G2_S2_UNITS[unitIdx];
    if (lessonIdx < u.lessons.length - 1) openLesson(unitIdx, lessonIdx + 1);
    else if (unitIdx < SCIENCE_G2_S2_UNITS.length - 1) openLesson(unitIdx + 1, 0);
    else setActive(null);
  };
  const backToOverview = () => { setActive(null); setDone(false); };

  if (active && !done && exercise) {
    const unit = SCIENCE_G2_S2_UNITS[active.unitIdx];
    ...
    return (
      <div className="space-y-6" dir="rtl">
        <PageHeader
          ...
          breadcrumbs={[{ label: t('home'), path: '/' }, { label: 'العلوم — الصف الثاني ف٢', path: '/science-grade2-s2' }, { label: lesson.title_ar }]}
        />
  ...
  if (active && done) {
    ...
    return (
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-xl mx-auto space-y-5 pt-8" dir="rtl">
        <div className="rounded-2xl p-8 text-center bg-gradient-to-br from-[#0e7490] to-[#109673] text-white">
  ...
  return (
    <InteractiveBankBrowser
      units={SCIENCE_G2_S2_UNITS}
      unitTheme={UNIT_THEME}
      selectedUnit={selectedUnit}
      onSelectUnit={setSelectedUnit}
      onPickLesson={openLesson}
      banner="https://images.unsplash.com/photo-1532094349884-543bc11b234d?w=1200&q=80"
      title="العلوم — الصف الثاني الابتدائي · ف٢"
      subtitle={`بنك أسئلة تفاعلي وفق الكتاب الرسمي (${toAr(TOTAL_UNITS_SCI_G2S2)} وحدات · ${toAr(TOTAL_LESSONS_SCI_G2S2)} دروس · ${toAr(TOTAL_QUESTIONS_SCI_G2S2)} سؤالًا)`}
      bankLabel="علوم — الصف الثاني ف٢"
      homeLabel={t('home')}
      backTo="/question-bank/grade_2/second"
      toAr={toAr}
      stats={[
        { n: toAr(TOTAL_UNITS_SCI_G2S2), l: 'وحدات' },
        { n: toAr(TOTAL_LESSONS_SCI_G2S2), l: 'درسًا' },
        { n: toAr(TOTAL_QUESTIONS_SCI_G2S2), l: 'سؤال' },
      ]}

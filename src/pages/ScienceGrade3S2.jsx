...
import {
  SCIENCE_G3_S2_UNITS, TOTAL_UNITS_SCI_G3S2, TOTAL_LESSONS_SCI_G3S2, TOTAL_QUESTIONS_SCI_G3S2,
} from '@/lib/scienceGrade3S2';
...
function ExerciseRenderer({ exercise, isAnswered, onAnswer }) {
  ...
  if (ex.type === 'matching')  return <MatchingQuestion exercise={ex} lang="ar" isAnswered={isAnswered} onAnswer={onAnswer} />;
  if (ex.type === 'classify')  return <ClassifyQuestion exercise={ex} lang="ar" isAnswered={isAnswered} onAnswer={onAnswer} />;
  if (ex.type === 'ordering')  return <OrderingQuestion exercise={ex} lang="ar" isAnswered={isAnswered} onAnswer={onAnswer} />;
...
export default function ScienceGrade3S2() {
  ...
  const exercises = useMemo(() => {
    if (!active) return [];
    return SCIENCE_G3_S2_UNITS[active.unitIdx].lessons[active.lessonIdx].questions;
  }, [active]);
  ...
  const nextLesson = () => {
    ...
    const u = SCIENCE_G3_S2_UNITS[unitIdx];
    if (lessonIdx < u.lessons.length - 1) openLesson(unitIdx, lessonIdx + 1);
    else if (unitIdx < SCIENCE_G3_S2_UNITS.length - 1) openLesson(unitIdx + 1, 0);
    else setActive(null);
  };
  const backToOverview = () => { setActive(null); setDone(false); };

  if (active && !done && exercise) {
    const unit = SCIENCE_G3_S2_UNITS[active.unitIdx];
    ...
    return (
      <div className="space-y-6" dir="rtl">
        <PageHeader
          ...
          breadcrumbs={[
            { label: t('home'), path: '/' },
            { label: 'العلوم — الصف الثالث ف٢', path: '/science-grade3-s2' },
        ...
        <div className="max-w-2xl mx-auto space-y-5">
          ...
          {/* صورة السؤال (إذا كانت موجودة في السؤال وليس داخل الخيارات) */}
          {exercise.image_url && exercise.type !== 'multiple_choice' && (
            <div className="rounded-xl overflow-hidden border border-slate-200">
              <img src={exercise.image_url} alt="صورة السؤال" className="w-full h-52 object-cover" />
            </div>
          )}
  ...
  if (active && done) {
    ...
    return (
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-xl mx-auto space-y-5 pt-8" dir="rtl">
        <div className="rounded-2xl p-8 text-center bg-gradient-to-br from-sky-600 to-teal-600 text-white">
  ...
  return (
    <InteractiveBankBrowser
      ...
      title="العلوم — الصف الثالث الابتدائي · ف٢"
      subtitle={`بنك أسئلة تفاعلي مع صور من الكتاب (${toAr(TOTAL_UNITS_SCI_G3S2)} وحدات · ${toAr(TOTAL_LESSONS_SCI_G3S2)} دروس · ${toAr(TOTAL_QUESTIONS_SCI_G3S2)} سؤالًا)`}
      bankLabel="علوم — الصف الثالث ف٢"
      homeLabel={t('home')}
      backTo="/question-bank/grade_3/second"
      toAr={toAr}
      stats={[
        { n: toAr(TOTAL_UNITS_SCI_G3S2), l: 'وحدات' },
        { n: toAr(TOTAL_LESSONS_SCI_G3S2), l: 'درسًا' },
        { n: toAr(TOTAL_QUESTIONS_SCI_G3S2), l: 'سؤال' },
      ]}

...
import { ChevronRight, CheckCircle2, XCircle, BookOpen, Star, RotateCcw, Home, Trophy, ArrowRight, Lightbulb } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { SCIENCE_G5_S2_UNITS, SCIENCE_G5_S2_TOTAL } from '@/lib/scienceGrade5S2Bank';
import WrongAnswerTeacher from '@/components/aptitude/WrongAnswerTeacher';

function InfoCard({ q, onNext }) {
  return (
    <div className="space-y-4">
      <div className="rounded-xl bg-sky-50 border-2 border-sky-200 p-4 flex items-start gap-3">
        <div className="flex-shrink-0 w-8 h-8 rounded-full bg-sky-500 flex items-center justify-center">
          <Lightbulb className="w-4 h-4 text-white" />
        </div>
        <div>
          <p className="text-xs font-bold text-sky-700 mb-1">معلومة من الكتاب</p>
          <p dir="rtl" className="text-sm text-sky-900 leading-relaxed">{q.question_ar}</p>
        </div>
      </div>
      <Button onClick={onNext} className="w-full bg-sky-600 hover:bg-sky-700">فهمت ← السؤال التالي <ArrowRight className="w-4 h-4" /></Button>
    </div>
  );
}
...
function QuestionRenderer({ q, onAnswer, isAnswered }) {
  ...
  const [orderItems] = useState(() => q.type === 'ordering' ? [...q.items_ar].sort(() => Math.random() - 0.5) : null);
  ...
  const handleClassifyPick = (binIdx) => {
    ...
    setDragItems(newItems); setBins(newBins);
  ...
  const moveItem = (from, to) => {
    const arr = [...orderedItems]; const [m] = arr.splice(from, 1); arr.splice(to, 0, m); setOrderedItems(arr);
  };
  const checkOrder = () => {
    const correct = q.correct_order ? q.correct_order.map(i => q.items_ar[i]) : q.items_ar;
    setSelected('checked'); onAnswer(orderedItems.join('|') === correct.join('|'));
  };
          ...
          return (
            <button key={label} disabled={show} onClick={() => handleTF(val)}
              className={`flex-1 py-4 rounded-xl border-2 text-base font-bold transition-all
                ${!show ? 'bg-white border-slate-200 hover:border-sky-400 cursor-pointer' : ''}
                ${show && isCorrect ? 'bg-emerald-50 border-emerald-400 text-emerald-800' : ''}
                ${show && isPicked && !isCorrect ? 'bg-red-50 border-red-400 text-red-800' : ''}
    ...
    return (
      <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2 justify-center mt-3">
              {q.categories_ar.map((cat, i) => (
                <button key={i} onClick={() => handleClassifyPick(i)}
                  className={`px-4 py-2 rounded-lg text-white text-sm font-bold transition-colors ${i === 0 ? 'bg-sky-500 hover:bg-sky-600' : i === 1 ? 'bg-orange-500 hover:bg-orange-600' : 'bg-purple-500 hover:bg-purple-600'}`}>
                  {cat}
                </button>
              ))}
            </div>
        ...
        <div className={`grid gap-3 ${q.categories_ar.length === 3 ? 'grid-cols-3' : 'grid-cols-2'}`}>
          {bins && bins.map((bin, i) => (
            <div key={i} className={`rounded-xl border-2 p-3 min-h-[60px] ${i === 0 ? 'border-sky-200 bg-sky-50' : i === 1 ? 'border-orange-200 bg-orange-50' : 'border-purple-200 bg-purple-50'}`}>
              <p className={`text-xs font-bold mb-2 ${i === 0 ? 'text-sky-700' : i === 1 ? 'text-orange-700' : 'text-purple-700'}`}>{q.categories_ar[i]}</p>
            ...
            {q.pairs_ar.map((p, i) => (
              <button key={i} onClick={() => handleMatchPick('right', i)}
                className={`w-full text-right px-3 py-2 rounded-lg border-2 text-xs font-medium transition-all
                  ${matchSel?.side === 'right' && matchSel?.idx === i ? 'border-sky-500 bg-sky-50' : 'border-slate-200 bg-white hover:border-sky-300'}
          ...
          <div className="space-y-2">
            {q.pairs_ar.map((p, i) => (
              <button key={i} onClick={() => handleMatchPick('left', i)}
                className={`w-full text-right px-3 py-2 rounded-lg border-2 text-xs font-medium transition-all
                  ${matchSel?.side === 'left' && matchSel?.idx === i ? 'border-sky-500 bg-sky-50' : 'border-slate-200 bg-white hover:border-sky-300'}
  ...
  if (q.type === 'ordering' && orderedItems) {
      ...
      <div className="space-y-3">
        ...
        {orderedItems.map((item, i) => (
          <div key={item} className="flex items-center gap-2 bg-white border-2 border-slate-200 rounded-xl px-3 py-2">
            <span className="w-6 h-6 rounded-full bg-sky-100 flex items-center justify-center text-xs font-bold text-sky-700 flex-shrink-0">{i + 1}</span>
            ...
            <div className="flex flex-col gap-0.5">
              ...
              <button disabled={i === orderedItems.length - 1 || checked} onClick={() => moveItem(i, i + 1)} className="text-slate-400 hover:text-slate-700 disabled:opacity-30 text-xs">▼</button>
            </div>
          </div>
        ))}
        {!checked && <Button onClick={checkOrder} className="w-full bg-sky-600 hover:bg-sky-700">تحقق من الترتيب ✓</Button>}
      </div>
    );
  }
  ...
  const handleAnswer = (correct) => {
    ...
    setTimeout(() => {
      if (containerRef.current)
        window.scrollTo({ top: containerRef.current.getBoundingClientRect().top + window.scrollY - 80, behavior: 'smooth' });
    }, 300);
  };
    ...
    return (
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center py-10 px-4 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          ...
          <Button onClick={onBack} className="bg-sky-600 hover:bg-sky-700 gap-2"><ArrowRight className="w-4 h-4" />العودة</Button>
        </div>
      </motion.div>
    );
  }
    ...
    <div ref={containerRef} className="space-y-4">
      <div className="flex items-center gap-3">
        <div className="flex-1 bg-slate-100 rounded-full h-2">
          <div className="bg-sky-500 h-2 rounded-full transition-all duration-500" style={{ width: `${(idx / questions.length) * 100}%` }} />
        </div>
        <span className="text-xs font-medium text-slate-500">{idx + 1} / {questions.length}</span>
        <span className="text-xs font-bold text-sky-600">✅ {score}</span>
      </div>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={q.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
          ...
          {isInfo ? (
            <InfoCard q={q} onNext={next} />
          ) : (
            <>
              <div className="flex items-start gap-2">
                <span className="text-xs bg-sky-100 text-sky-700 rounded-full px-2 py-0.5 font-bold flex-shrink-0 mt-0.5">{idx + 1}</span>
                <p className="text-base font-bold text-slate-800 leading-relaxed">{q.question_ar}</p>
              </div>
              {q.image_url && (
                <div className="rounded-xl overflow-hidden border border-slate-100 bg-slate-50">
                  <img src={q.image_url} alt="" className="w-full max-h-48 object-contain" />
                </div>
              )}
              ...
              {answered && (
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
                  {lastCorrect ? (
                    ...
                    <WrongAnswerTeacher explanation={q.explanation_ar} goldenRule={q.explanation_ar} lang="ar" isRTL={true} />
                  )}
                  <Button onClick={next} className="w-full mt-3 bg-sky-600 hover:bg-sky-700 gap-2">
...
export default function ScienceGrade5S2() {
  const [activeUnit, setActiveUnit] = useState(null);
  const [activeLesson, setActiveLesson] = useState(null);

  if (activeUnit !== null && activeLesson !== null) {
    const unit = SCIENCE_G5_S2_UNITS[activeUnit];
    const lesson = unit.lessons[activeLesson];
    return (
      <div className="min-h-screen bg-gradient-to-br from-sky-50 to-cyan-50 p-4 sm:p-6" dir="rtl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {unit.lessons.map((lesson, i) => (
              <motion.button key={lesson.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                ...
                className="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 text-right hover:shadow-md hover:border-sky-200 transition-all flex items-center gap-3">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-sky-50 to-cyan-50" dir="rtl">
      <div className="relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #0369a1 0%, #0891b2 60%, #164e63 100%)' }}>
        ...
        <div className="relative max-w-3xl mx-auto px-4 py-10 sm:py-14 text-center text-white">
          ...
          <h1 className="text-2xl sm:text-3xl font-black mb-2">علوم الصف الخامس</h1>
          <p className="text-white/80 text-sm">الفصل الثاني | الطاقة والآلات البسيطة · الصوت والضوء</p>
          <div className="flex justify-center gap-4 mt-5 text-xs flex-wrap">
            <div className="bg-white/10 rounded-xl px-4 py-2">
              <p className="font-bold text-lg">{SCIENCE_G5_S2_UNITS.length}</p>
              <p className="text-white/70">وحدات</p>
            </div>
            <div className="bg-white/10 rounded-xl px-4 py-2">
              <p className="font-bold text-lg">{SCIENCE_G5_S2_UNITS.reduce((s, u) => s + u.lessons.length, 0)}</p>
              <p className="text-white/70">دروس</p>
            </div>
            <div className="bg-white/10 rounded-xl px-4 py-2">
              <p className="font-bold text-lg">{SCIENCE_G5_S2_TOTAL}</p>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-4">
        {SCIENCE_G5_S2_UNITS.map((unit, i) => (
          <motion.button key={unit.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            transition={{ delay: i * 0.1 }} onClick={() => setActiveUnit(i)}
            className="w-full bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-md hover:border-sky-200 transition-all text-right">
            <div className="flex items-center gap-4 p-5">
              <div className={`w-14 h-14 rounded-xl flex-shrink-0 bg-gradient-to-br ${unit.color} flex items-center justify-center text-3xl`}>
                {unit.icon}
              </div>
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 mt-2">
                  <span className="text-xs bg-sky-100 text-sky-700 rounded-full px-2 py-0.5">{unit.lessons.length} دروس</span>

...
import { SCIENCE_G5_S2_UNITS, SCIENCE_G5_S2_TOTAL } from '@/lib/scienceG5S2';
    ...
    <div className="space-y-4">
      <div className="rounded-xl bg-emerald-50 border-2 border-emerald-200 p-4 flex items-start gap-3">
        ...
        <div>
          ...
          <p dir="rtl" className="text-sm text-emerald-900 leading-relaxed font-medium">{q.question_ar}</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {!done && (
          <div className="rounded-xl border-2 border-dashed border-amber-300 bg-amber-50 p-4 text-center">
            ...
            <span className="text-sm font-bold text-slate-800 bg-white px-4 py-2 rounded-lg shadow-sm border">{dragItems[0].text}</span>
        ...
        <motion.div key={q.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
          ...
          {isInfo ? <InfoCard q={q} onNext={next} /> : (
            <>
              ...
              {q.image && <div className="rounded-xl overflow-hidden border border-slate-100 bg-slate-50"><img src={q.image} alt="" className="w-full max-h-48 object-contain" /></div>}
                    ...
                    <div className="space-y-2">
                      ...
                      {q.golden_rule && (
                        <div className="rounded-xl bg-amber-50 border-2 border-amber-300 p-3 flex items-start gap-2">
                          ...
                          <p className="text-xs font-medium text-amber-900">{q.golden_rule}</p>
                        </div>
                      )}
                    </div>
                  ) : (
...
export default function ScienceGrade5S2Bank() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-teal-50" dir="rtl">
      <div className="relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #065f46 0%, #059669 60%, #047857 100%)' }}>
        ...
        <div className="relative max-w-3xl mx-auto px-4 py-10 sm:py-14 text-center text-white">
          ...
          <p className="text-white/80 text-sm">الفصل الدراسي الثاني | تنوع الحياة والأنظمة البيئية والأرض ومواردها</p>
          <div className="flex justify-center gap-4 mt-5 text-xs flex-wrap">
            <div className="bg-white/10 rounded-xl px-4 py-2"><p className="font-bold text-lg">{SCIENCE_G5_S2_UNITS.length}</p><p className="text-white/70">وحدات</p></div>
            <div className="bg-white/10 rounded-xl px-4 py-2"><p className="font-bold text-lg">{SCIENCE_G5_S2_UNITS.reduce((s, u) => s + u.lessons.length, 0)}</p><p className="text-white/70">دروس</p></div>
            <div className="bg-white/10 rounded-xl px-4 py-2"><p className="font-bold text-lg">{SCIENCE_G5_S2_TOTAL}</p><p className="text-white/70">سؤالاً</p></div>
          </div>
        </div>
      </div>

...
import { SOCIAL_G5_S1_UNITS, SOCIAL_G5_S1_TOTAL } from '@/lib/socialGrade5S1Bank';
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        {opts.map((opt) => {
          ...
          let cls = 'border-2 rounded-xl py-4 text-lg font-bold transition-all ';
          ...
          return (
            <button key={opt} disabled={answered} onClick={() => onAnswer(opt)} className={cls}>
              {opt === 'نعم' ? '✅ نعم' : '❌ لا'}
            </button>
          );
        })}
...
function OrderingQ({ q, onAnswer, answered }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {!checked && !answered && (
        <Button onClick={check} className="w-full">تحقَّق من الترتيب</Button>
      )}
    </div>
  );
}
...
function MatchingQ({ q, onAnswer, answered }) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm font-bold text-slate-700">{q.q}</p>
      <p className="text-xs text-slate-400">💡 اضغط على العمود الأيسر ثم اختَر ما يناسبه من العمود الأيمن</p>
          ...
          {leftItems.map((l) => {
            ...
            return (
              <button key={l} onClick={() => handleLeft(l)} disabled={checked}
                className={`w-full text-xs px-2 py-2 rounded-lg border-2 text-right transition-all ${isSelected ? 'border-blue-500 bg-blue-50' : isConnected ? 'border-emerald-400 bg-emerald-50' : 'border-slate-200 hover:border-blue-300'}`}>
                {l} {isConnected && <span className="text-emerald-600">← {connections[l]}</span>}
              </button>
            );
          })}
        </div>
        ...
        <div className="flex flex-wrap gap-2 p-3 bg-slate-50 rounded-xl border border-dashed border-slate-300">
          {pool.map((item) => (
            <div key={item.text} className="relative group">
              ...
              <div className="absolute top-7 right-0 z-10 hidden group-hover:flex flex-col gap-1 bg-white border border-slate-200 rounded-lg shadow-lg p-1 w-36">
                {q.categories.map((cat) => (
                  <button key={cat} onClick={() => assign(item, cat)}
                    className="text-xs px-2 py-1 rounded hover:bg-blue-50 text-right">{cat}</button>
                ))}
              </div>
            </div>
          ))}
...
function WordOrderQ({ q, onAnswer, answered }) {
  ...
  const check = () => {
    ...
    const isCorrect = sentence === q.correct_sentence || slots.join(' ') === q.correct_sentence.replace(/\./g, '');
    setChecked(true);
    onAnswer(isCorrect ? '1' : '0');
  };
  const reset = () => {
    ...
    <div className="space-y-3">
      ...
      <div className="min-h-[48px] p-3 bg-slate-50 border-2 border-dashed border-slate-300 rounded-xl flex flex-wrap gap-2">
        {slots.map((w, i) => (
          <button key={i} onClick={() => removeWord(i)} disabled={checked}
            className={`px-3 py-1.5 rounded-lg text-sm font-bold border-2 ${checked ? (slots.join(' ') === q.correct_sentence.replace(/\./g, '') || slots.join(' ') === q.correct_sentence ? 'border-emerald-400 bg-emerald-50 text-emerald-800' : 'border-rose-400 bg-rose-50 text-rose-800') : 'border-blue-400 bg-blue-50 text-blue-800'}`}>
            {w}
          </button>
        ))}
      ...
      <div className="flex flex-wrap gap-2">
        {pool.map((w, i) => (
          <button key={i} onClick={() => addWord(w)}
            className="px-3 py-1.5 rounded-lg text-sm font-medium border-2 border-slate-200 bg-white hover:border-blue-400 hover:bg-blue-50 transition-all">
            {w}
          </button>
        ))}
      </div>
      <div className="flex gap-2">
        {!checked && pool.length === 0 && (
          <Button onClick={check} className="flex-1">تحقَّق من الجملة</Button>
        )}
...
const TYPE_LABEL = {
  ...
  mc: { label: 'اختيار من متعدد', color: 'bg-blue-100 text-blue-700' },
...
// ─── الصفحة الرئيسية ───────────────────────────────────────
export default function SocialGrade5S1() {
  ...
  const unit = active ? SOCIAL_G5_S1_UNITS[active.unitIdx] : null;
  ...
  const next = () => {
    if (qIdx < questions.length - 1) {
      setQIdx((i) => i + 1); setAnswered(false); setUserAns(null); setCorrect(false);
    } else { setDone(true); }
  };
    ...
    return (
      <div className="space-y-6" dir="rtl">
        <PageHeader
          ...
          breadcrumbs={[
            { label: t('home'), path: '/' },
            { label: 'الدراسات الاجتماعية — الصف الخامس ف١', path: '/social-grade5-s1' },
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div className="flex items-center gap-2 pb-2 border-b border-slate-100">
                ...
                <span className="text-xs text-slate-400">من كتاب الدراسات الاجتماعية — الصف الخامس الابتدائي — الفصل الأول</span>
              </div>
          ...
          {answered && (
            <>
              ...
              <Button onClick={next} className="w-full gap-2 py-6 text-base font-bold">
                {qIdx < questions.length - 1 ? 'السؤال التَّالي' : 'إنهاءُ الدَّرس'}
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div className="rounded-2xl p-8 text-center bg-gradient-to-br from-[#263D84] to-[#3E57A4] text-white">
          ...
          <p className="text-white/80">
            {pct >= 80 ? 'إتقانٌ ممتازٌ 🏆' : pct >= 50 ? 'جيِّدٌ، واصِل المُراجعة 💪' : 'يحتاجُ مراجعةً — أعِد المحاولة 📖'}
          </p>
        ...
        <div className="flex gap-3">
          ...
          <Button onClick={backToUnits} className="flex-1 gap-2">
            وحداتٌ أخرى <Arrow className="w-4 h-4" />
          </Button>
        </div>
      </motion.div>
    );
  ...
  return (
    <div className="space-y-8" dir="rtl">
      <PageHeader
        title="الدراسات الاجتماعية — الصف الخامس الابتدائي · الفصل الأول"
        subtitle={`${toAr(SOCIAL_G5_S1_UNITS.length)} وحداتٍ · ${toAr(SOCIAL_G5_S1_TOTAL)} سؤالًا — الخلفاء الراشدون · الدول الإسلامية · الأمن · الجغرافيا · مؤسسات الدولة · الخرائط · الموارد · الاقتصاد`}
        breadcrumbs={[{ label: t('home'), path: '/' }, { label: 'الدراسات الاجتماعية — الصف الخامس ف١' }]}
      />

      <div className="max-w-4xl mx-auto px-4 space-y-6">
        <div className="grid grid-cols-3 gap-3">
          {[
            { n: toAr(SOCIAL_G5_S1_UNITS.length), l: 'وحدة' },
            { n: toAr(SOCIAL_G5_S1_UNITS.reduce((s, u) => s + u.lessons.length, 0)), l: 'درسًا' },
            { n: toAr(SOCIAL_G5_S1_TOTAL), l: 'سؤالًا' },
          ].map((s) => (
        ...
        {SOCIAL_G5_S1_UNITS.map((unit, ui) => (

...
import { SOCIAL_G6_S2_UNITS as UNITS_P1, SOCIAL_G6_S2_TOTAL as TOTAL_P1 } from '@/lib/socialGrade6S2Bank';
import { SOCIAL_G6_S2_UNITS_PART2, SOCIAL_G6_S2_TOTAL_PART2 } from '@/lib/socialGrade6S2Bank_Part2';
import { SOCIAL_G6_S2_UNITS_PART3, SOCIAL_G6_S2_TOTAL_PART3 } from '@/lib/socialGrade6S2Bank_Part3';
import WrongAnswerTeacher from '@/components/aptitude/WrongAnswerTeacher';

const SOCIAL_G6_S2_UNITS = [...UNITS_P1, ...SOCIAL_G6_S2_UNITS_PART2, ...SOCIAL_G6_S2_UNITS_PART3];
const SOCIAL_G6_S2_TOTAL = TOTAL_P1 + SOCIAL_G6_S2_TOTAL_PART2 + SOCIAL_G6_S2_TOTAL_PART3;
      ...
      <div className="space-y-3">
        ...
        {orderItems.map((item, i) => (
          <div key={item} className="flex items-center gap-2 bg-white border-2 border-slate-200 rounded-xl px-3 py-2">
            <span className="w-6 h-6 rounded-full bg-blue-100 flex items-center justify-center text-xs font-bold text-blue-700 flex-shrink-0">{i + 1}</span>
        ...
        {!checked && <Button onClick={checkOrder} className="w-full bg-blue-600 hover:bg-blue-700">تحقق من الترتيب ✓</Button>}
      </div>
    );
  }
    ...
    <div ref={containerRef} className="space-y-4">
      <div className="flex items-center gap-3">
        <div className="flex-1 bg-slate-100 rounded-full h-2">
          <div className="bg-blue-500 h-2 rounded-full transition-all duration-500" style={{ width: `${(idx / questions.length) * 100}%` }} />
        </div>
        ...
        <motion.div key={q.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
          ...
          {answered && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <Button onClick={next} className="w-full mt-3 bg-blue-600 hover:bg-blue-700 gap-2">
...
export default function SocialGrade6S2() {
  const [activeUnit, setActiveUnit] = useState(null);
  const [activeLesson, setActiveLesson] = useState(null);

  if (activeUnit !== null && activeLesson !== null) {
    const unit = SOCIAL_G6_S2_UNITS[activeUnit];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50" dir="rtl">
      <div className="relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #1e40af 0%, #2563eb 60%, #4338ca 100%)' }}>
        ...
        <div className="relative max-w-3xl mx-auto px-4 py-10 sm:py-14 text-center text-white">
          ...
          <p className="text-white/80 text-sm">التاريخ والجغرافيا والاقتصاد · ست وحدات تفاعلية</p>
          <div className="flex justify-center gap-4 mt-5 text-xs flex-wrap">
            <div className="bg-white/10 rounded-xl px-4 py-2">
              <p className="font-bold text-lg">{SOCIAL_G6_S2_UNITS.length}</p>
              <p className="text-white/70">وحدات</p>
            </div>
            <div className="bg-white/10 rounded-xl px-4 py-2">
              <p className="font-bold text-lg">{SOCIAL_G6_S2_UNITS.reduce((s, u) => s + u.lessons.length, 0)}</p>
              <p className="text-white/70">دروس</p>
            </div>
            <div className="bg-white/10 rounded-xl px-4 py-2">
              <p className="font-bold text-lg">{SOCIAL_G6_S2_TOTAL}</p>

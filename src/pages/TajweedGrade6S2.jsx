...
import { TALAWAH_G6_S2_UNITS as TALAWAH_UNITS, TALAWAH_G6_S2_TOTAL as TALAWAH_TOTAL } from '@/lib/talawahGrade6S2Bank';
import { TAJWEED_G6_S2_UNITS as TAJWEED_UNITS_1, TAJWEED_G6_S2_TOTAL as TAJWEED_TOTAL_1 } from '@/lib/tajweedGrade6S2Bank';
import { TAJWEED_G6_S2_UNITS_PART2, TAJWEED_G6_S2_TOTAL_PART2 } from '@/lib/tajweedGrade6S2Bank_Part2';
import { TAJWEED_G6_S2_UNITS_PART3, TAJWEED_G6_S2_TOTAL_PART3 } from '@/lib/tajweedGrade6S2Bank_Part3';
import { TAJWEED_G6_S2_UNITS_PART4, TAJWEED_G6_S2_TOTAL_PART4 } from '@/lib/tajweedGrade6S2Bank_Part4';
import { TAJWEED_G6_S2_UNITS_PART5, TAJWEED_G6_S2_TOTAL_PART5 } from '@/lib/tajweedGrade6S2Bank_Part5';

const TAJWEED_G6_S2_UNITS = [...TAJWEED_UNITS_1, ...TAJWEED_G6_S2_UNITS_PART2, ...TAJWEED_G6_S2_UNITS_PART3, ...TAJWEED_G6_S2_UNITS_PART4, ...TAJWEED_G6_S2_UNITS_PART5];
const TAJWEED_G6_S2_TOTAL = TAJWEED_TOTAL_1 + TAJWEED_G6_S2_TOTAL_PART2 + TAJWEED_G6_S2_TOTAL_PART3 + TAJWEED_G6_S2_TOTAL_PART4 + TAJWEED_G6_S2_TOTAL_PART5;

const TALAWAH_G6_S2_UNITS = [...TAJWEED_G6_S2_UNITS, ...TALAWAH_UNITS];
const TALAWAH_G6_S2_TOTAL = TAJWEED_G6_S2_TOTAL + TALAWAH_TOTAL;
...
function QuestionRenderer({ q, onAnswer, isAnswered }) {
  ...
  if (q.type === 'classify' && dragItems !== null) {
    ...
    const catColors = ['border-sky-200 bg-sky-50', 'border-orange-200 bg-orange-50', 'border-purple-200 bg-purple-50'];
    const btnColors = ['bg-sky-500 hover:bg-sky-600', 'bg-orange-500 hover:bg-orange-600', 'bg-purple-500 hover:bg-purple-600'];
    return (
      <div className="space-y-4">
        {!done && (
          <div className="rounded-xl border-2 border-dashed border-amber-300 bg-amber-50 p-4 text-center">
            <p className="text-xs text-amber-700 font-bold mb-2">صنّف هذا المثال:</p>
            <span className="text-base font-bold text-slate-800 bg-white px-4 py-2 rounded-lg shadow-sm border">{dragItems[0].text}</span>
            <div className="flex flex-wrap gap-2 justify-center mt-3">
              {q.categories_ar.map((cat, i) => (
                <button key={i} onClick={() => handleClassifyPick(i)}
                  className={`px-4 py-2 rounded-lg text-white text-sm font-bold transition-colors ${btnColors[i % 3]}`}>
                  {cat}
                </button>
              ))}
            </div>
        ...
        <div className={`grid gap-3 ${q.categories_ar.length === 3 ? 'grid-cols-3' : 'grid-cols-2'}`}>
          {bins && bins.map((bin, i) => (
            <div key={i} className={`rounded-xl border-2 p-3 min-h-[60px] ${catColors[i % 3]}`}>
              <p className="text-xs font-bold mb-2 text-slate-600">{q.categories_ar[i]}</p>
              <div className="flex flex-wrap gap-1">
                {bin.map((it, j) => <span key={j} className="text-xs bg-white border rounded-full px-2 py-0.5 text-slate-700">{it.text}</span>)}
              </div>
            </div>
          ))}
        </div>
            ...
            {q.pairs_ar.map((p, i) => (
              <button key={i} onClick={() => handleMatchPick('right', i)}
                className={`w-full text-right px-3 py-2 rounded-lg border-2 text-xs font-medium transition-all
                  ...
                  ${connections[i] !== undefined ? 'border-emerald-400 bg-emerald-50' : ''}`}>{p.right}</button>
            ))}
          </div>
          <div className="space-y-2">
            {q.pairs_ar.map((p, i) => (
              <button key={i} onClick={() => handleMatchPick('left', i)}
                className={`w-full text-right px-3 py-2 rounded-lg border-2 text-xs font-medium transition-all
                  ...
                  ${Object.values(connections).includes(i) ? 'border-emerald-400 bg-emerald-50' : ''}`}>{p.left}</button>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        {orderItems.map((item, i) => (
          <div key={item} className="flex items-center gap-2 bg-white border-2 border-slate-200 rounded-xl px-3 py-2">
            <span className="w-6 h-6 rounded-full bg-teal-100 flex items-center justify-center text-xs font-bold text-teal-700 flex-shrink-0">{i + 1}</span>
            <span className="flex-1 text-xs sm:text-sm font-medium text-slate-800 text-right">{item}</span>
        ...
        {!checked && <Button onClick={checkOrder} className="w-full bg-teal-600 hover:bg-teal-700">تحقق من الترتيب ✓</Button>}
      </div>
    );
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <div className="flex-1 bg-slate-100 rounded-full h-2">
          <div className="bg-teal-500 h-2 rounded-full transition-all duration-500" style={{ width: `${(idx / questions.length) * 100}%` }} />
        </div>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={q.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
          className="bg-white rounded-2xl shadow-sm border border-teal-100 p-5 space-y-4">
          <div className="flex items-start gap-2">
            <span className="text-xs bg-teal-100 text-teal-700 rounded-full px-2 py-0.5 font-bold flex-shrink-0 mt-0.5">{idx + 1}</span>
          ...
          {answered && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
              {lastCorrect ? (
                <div className="space-y-2">
                  ...
                  {q.explanation_ar && (
                    <div className="rounded-xl bg-teal-50 border border-teal-100 p-3">
                      <p className="text-xs font-bold text-teal-700 mb-1">💡 الشرح:</p>
                      <p className="text-sm text-teal-900 leading-relaxed">{q.explanation_ar}</p>
                      {q.golden_rule_ar && (
                ...
                <WrongAnswerTeacher explanation={q.explanation_ar} goldenRule={q.golden_rule_ar} lang="ar" isRTL={true} isCorrect={false} />
              )}
              <Button onClick={next} className="w-full mt-3 bg-teal-600 hover:bg-teal-700 gap-2">
...
export default function TajweedGrade6S2() {
  const [activeUnit, setActiveUnit] = useState(null);
  const [activeLesson, setActiveLesson] = useState(null);

  if (activeUnit !== null && activeLesson !== null) {
    const unit = TALAWAH_G6_S2_UNITS[activeUnit];
        ...
        <div className="max-w-2xl mx-auto space-y-5">
          <div className={`rounded-2xl p-6 text-white bg-gradient-to-r ${unit.color}`}>
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg font-bold">{unit.title_ar}</h1>
          ...
          <div className="grid grid-cols-1 gap-4">
            {unit.lessons.map((lesson, i) => (
              <motion.button key={lesson.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                ...
                className="bg-white rounded-2xl shadow-sm border border-teal-100 p-4 text-right hover:shadow-md hover:border-teal-300 transition-all flex items-center gap-3">
                <div className="w-12 h-12 rounded-xl flex-shrink-0 bg-teal-50 border border-teal-200 flex items-center justify-center">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-teal-50 to-cyan-50" dir="rtl">
      <div className="relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #0f766e 0%, #0d9488 50%, #0891b2 100%)' }}>
        ...
        <div className="relative max-w-3xl mx-auto px-4 py-10 sm:py-14 text-center text-white">
          ...
          <h1 className="text-2xl sm:text-3xl font-black mb-2">التلاوة والتجويد — الصف السادس</h1>
          <p className="text-white/80 text-sm">الفصل الدراسي الثاني · أحكام التجويد وتلاوة سور القرآن الكريم</p>
          <div className="flex justify-center gap-4 mt-5 text-xs flex-wrap">
            <div className="bg-white/10 rounded-xl px-4 py-2 border border-white/20">
              <p className="font-bold text-lg">{TALAWAH_G6_S2_UNITS.length}</p>
              <p className="text-white/70">وحدات</p>
            </div>
            <div className="bg-white/10 rounded-xl px-4 py-2 border border-white/20">
              <p className="font-bold text-lg">{TALAWAH_G6_S2_UNITS.reduce((s, u) => s + u.lessons.length, 0)}</p>
              <p className="text-white/70">دروس</p>
            </div>
            <div className="bg-white/10 rounded-xl px-4 py-2 border border-white/20">
              <p className="font-bold text-lg">{TALAWAH_G6_S2_TOTAL}</p>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-4">
        {TALAWAH_G6_S2_UNITS.map((unit, i) => (
          <motion.button key={unit.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            transition={{ delay: i * 0.08 }} onClick={() => setActiveUnit(i)}
            className="w-full bg-white rounded-2xl shadow-sm border border-teal-100 overflow-hidden hover:shadow-md hover:border-teal-300 transition-all text-right">
            <div className="flex items-center gap-4 p-5">
              <div className={`w-14 h-14 rounded-xl flex-shrink-0 bg-gradient-to-br ${unit.color} flex items-center justify-center text-2xl`}>
                {unit.icon}
              </div>
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 mt-2">
                  <span className="text-xs bg-teal-100 text-teal-700 rounded-full px-2 py-0.5">{unit.lessons.length} دروس</span>
                  <span className="text-xs bg-cyan-100 text-cyan-700 rounded-full px-2 py-0.5">{unit.lessons.reduce((s, l) => s + l.count, 0)} سؤالاً</span>
                </div>
              </div>

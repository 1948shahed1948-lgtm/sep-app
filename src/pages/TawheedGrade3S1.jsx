...
import {
  ...
  KeyRound, Heart, Feather, Users, Sunrise, Scale, Sparkles, Star, Mic, MessageCircle, Clock,
} from 'lucide-react';
...
import { TAWHEED_G3_UNITS, TAWHEED_G3_BANNER, TOTAL_LESSONS_G3, TOTAL_QUESTIONS_G3 } from '@/lib/tawheedGrade3S1';
import { withQuestionImages } from '@/lib/bankImagePools';
import { normalizeIslamicG3Batch } from '@/lib/islamicGrade3S1/normalize';
...
const TYPE_BADGE = {
  ...
  default:        { label: 'تمرين', color: 'bg-slate-100 text-slate-700' },
};
...
const LESSON_ICONS = { KeyRound, Heart, Feather, BookOpen, Users, Sunrise, Scale, Sparkles, Star, Mic, MessageCircle, Clock };
...
export default function TawheedGrade3S1() {
  ...
  const exercises = useMemo(() => {
    if (!active) return [];
    const raw = TAWHEED_G3_UNITS[active.unitIdx].lessons[active.lessonIdx].questions;
    return withQuestionImages(normalizeIslamicG3Batch(raw), 'tawheed');
  }, [active]);
  ...
  const nextLesson = () => {
    ...
    const u = TAWHEED_G3_UNITS[unitIdx];
    if (lessonIdx < u.lessons.length - 1) openLesson(unitIdx, lessonIdx + 1);
    else if (unitIdx < TAWHEED_G3_UNITS.length - 1) openLesson(unitIdx + 1, 0);
    else setActive(null);
  };
  const backToOverview = () => { setActive(null); setDone(false); };

  if (active && !done && exercise) {
    const unit = TAWHEED_G3_UNITS[active.unitIdx];
    ...
    return (
      <div className="space-y-6" dir="rtl">
        <PageHeader
          ...
          breadcrumbs={[{ label: t('home'), path: '/' }, { label: 'التوحيد والفقه — الصف الثالث', path: '/tawheed-grade3-s1' }, { label: lesson.title_ar }]}
        />
        <div className="max-w-2xl mx-auto space-y-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className={`text-xs font-bold px-3 py-1 rounded-full ${(TYPE_BADGE[exercise.type] || TYPE_BADGE.default).color}`}>{(TYPE_BADGE[exercise.type] || TYPE_BADGE.default).label}</span>
          </div>
  ...
  return (
    <InteractiveBankBrowser
      units={TAWHEED_G3_UNITS}
      unitTheme={UNIT_THEME}
      selectedUnit={selectedUnit}
      onSelectUnit={setSelectedUnit}
      onPickLesson={openLesson}
      banner={TAWHEED_G3_BANNER}
      title="التوحيد والفقه والسلوك — الصف الثالث الابتدائي"
      subtitle={`بنك أسئلة تفاعلي وفق الكتاب الرسمي (وزارة التعليم 1447/2025) · (${toArabicNumber(TAWHEED_G3_UNITS.length)} وحدات · ${toArabicNumber(TOTAL_LESSONS_G3)} دروس · ${toArabicNumber(TOTAL_QUESTIONS_G3)} سؤالًا)`}
      bankLabel="توحيد وفقه — الصف الثالث"
      homeLabel={t('home')}
      backTo="/"
      toAr={toArabicNumber}
      stats={[
        { n: toArabicNumber(TAWHEED_G3_UNITS.length), l: 'وحدات' },
        { n: toArabicNumber(TOTAL_LESSONS_G3), l: 'درسًا' },
        { n: toArabicNumber(TOTAL_QUESTIONS_G3), l: 'سؤال' },
      ]}
